...
학습된 sa_boundary_tagger.pt를 로드하여 번역문 경계 추론
"""
...
class SaBoundaryTaggerLoader:
    ...
    def __init__(self, model_path: Path = None, device: str = "cuda"):
        ...
        if model_path is None:
            model_path = Path(__file__).parent.parent / "models" / "sa_boundary_tagger.pt"
        
        if not model_path.exists():
            raise FileNotFoundError(f"SA 경계 모델 없음: {model_path}")
        ...
        print(f"✅ SA 경계 태거 로드 완료 (vocab={len(self.vocab)}, device={self.device})")
    ...
    def segment_text(self, text: str, threshold: float = 0.5, task: str = None, **kwargs) -> List[str]:
        """텍스트를 경계 위치에서 분할
...
def get_sa_boundary_tagger(model_path: Path = None, device: str = "cuda") -> SaBoundaryTaggerLoader:
    """SA 경계 태거 싱글톤 반환"""
    global _sa_tagger_instance
    
    if _sa_tagger_instance is None:
        _sa_tagger_instance = SaBoundaryTaggerLoader(model_path=model_path, device=device)
