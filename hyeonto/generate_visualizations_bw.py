...
def load_cluster_data(boundary: str = "sentence", k: int = 3) -> pd.DataFrame:
    """클러스터 데이터 로드"""
...
def generate_cluster_distribution_chart(
    df: pd.DataFrame, boundary: str, output_dir: Path
):
    """클러스터 분포 막대 차트 (흑백)"""
    ...
    legend_elements = [
        mpatches.Patch(
            facecolor=color, edgecolor="black", hatch=hatch, label=f"C{c}: {count:,}건"
        )
        for c, color, hatch, count in zip(
            clusters, GRAYSCALE_COLORS, GRAYSCALE_HATCHES, counts
        )
    ]
...
def generate_book_distribution_heatmap(
    df: pd.DataFrame, boundary: str, output_dir: Path
):
    """서적별 클러스터 분포 히트맵 (흑백)"""
    ...
    for i in range(len(pivot.index)):
        ...
        for j in range(len(pivot.columns)):
            ...
            ax.text(
                j, i, f"{val:.1f}", ha="center", va="center", color=color, fontsize=8
            )
...
def generate_umap_visualization(df: pd.DataFrame, boundary: str, output_dir: Path):
    ...
    try:
        ...
        vectorizer = TfidfVectorizer(
            max_features=500, analyzer="char", ngram_range=(1, 3)
        )
    ...
    ax.set_title(
        f"{boundary.capitalize()} UMAP 시각화 (K={len(clusters)}, n={sample_size:,})"
    )
...
def generate_marker_frequency_chart(df: pd.DataFrame, boundary: str, output_dir: Path):
    ...
    bars = ax.barh(
        range(len(marker_counts)),
        marker_counts.values,
        color=grays,
        edgecolor="black",
        linewidth=0.8,
    )
    ...
    for i, (bar, count) in enumerate(zip(bars, marker_counts.values)):
        ...
        ax.text(
            bar.get_width() + max(marker_counts) * 0.01,
...
def generate_cluster_marker_profile(df: pd.DataFrame, boundary: str, output_dir: Path):
    ...
    for idx, cluster_id in enumerate(clusters[:4]):
        ...
        bars = ax.barh(
            range(len(top_markers)),
            top_markers.values,
            color=GRAYSCALE_COLORS[idx],
            edgecolor="black",
            linewidth=0.8,
...
def main():
    ...
    sentence_df = load_cluster_data("sentence", 3)
    ...
    phrase_df = load_cluster_data("phrase", 3)
