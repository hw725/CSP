...
weight_sensitivity/run_validation_analysis.py의 결과를 시각화하는 HTML 파일 생성:
...
def generate_weight_sensitivity_html(output_path: Path):
    ...
    # 데이터 로드
    json_path = (
        VALIDATION_DIR / "weight_sensitivity" / "weight_sensitivity_summary.json"
    )
    ...
    def _scenario_row(s):
        avg = s["avg_weighted_canonicity"]
        mx = s["max_weighted_canonicity"]
        ent = s["avg_genre_entropy"]
        return f"<tr><td>{s['name']}</td><td>-</td><td>{avg:.2f}%</td><td>{mx:.2f}%</td><td>{ent:.4f}</td></tr>"

    scenario_rows = "\n            ".join(_scenario_row(s) for s in scenarios)
...
def generate_hypothesis_test_html(output_path: Path):
    ...
    verdict_class = (
        "verdict-pass" if null_h["verdict"] == "REJECTED" else "verdict-fail"
    )
...
if __name__ == "__main__":
    ...
    ws_html = (
        VALIDATION_DIR
        / "weight_sensitivity"
        / "weight_sensitivity_visualization_bw.html"
    )
