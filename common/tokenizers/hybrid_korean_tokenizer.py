...
class HybridKoreanTokenizer:
    ...
    def tokenize_korean_text(self, text: str, text_type: str = "translation") -> Dict[str, Any]:
        """
        한국어 텍스트 하이브리드 토큰화
        ...
        try:
            ...
            roberta_analysis = self.roberta_tokenizer.tokenize_text_with_hanja_priority(text)
    ...
    def split_semantic_units(self, text: str, min_length: int = 2) -> List[str]:
        ...
        try:
            ...
            # 폴백: 문장부호 기준 분할
            import re
            return [s.strip() for s in re.split(r'[.!?。]', text) if len(s.strip()) >= min_length]

# 전역 인스턴스
_hybrid_korean_tokenizer = None

def get_hybrid_korean_tokenizer(roberta_model: str = "klue/roberta-large",
                               kiwi_model: str = "base",
                               device: str = "auto") -> HybridKoreanTokenizer:
