...
def main():
    ...
    parser.add_argument(
        "--grayscale", action="store_true", help="컬러 + 흑백 버전 모두 생성"
    )

    parser.add_argument(
        "--only-grayscale", action="store_true", help="흑백 버전만 생성"
    )

    parser.add_argument(
        "--skip-umap", action="store_true", help="UMAP 임베딩 건너뛰기 (시간 절약)"
    )
    ...
    mode = (
        "only-grayscale"
        if args.only_grayscale
        else ("both" if args.grayscale else "color")
    )
    ...
    if not args.only_grayscale:
        ...
        if not args.skip_umap:
            ...
            if run_script(
                "analyze_embedding_overlay.py",
                [
                    "--sample",
                    "10000",
                    "--sentence-cache-npy",
                    str(BASE_DIR / "results" / "sentence_embeddings_cache.npy"),
                    "--phrase-cache-npy",
                    str(BASE_DIR / "results" / "phrase_embeddings_cache.npy"),
                ],
            ):
    ...
    if scripts_failed > 0:
        ...
        print(f"??  실패한 스크립트: {scripts_failed}개")

    print("\n? 생성된 시각화 위치:")

    print(f"   - 임베딩 오버레이: reports/k3_embedding_overlay_*.html")
