...
def build_sankey_data(
    labels_small: np.ndarray, labels_large: np.ndarray, k_small: int, k_large: int
):
    """Sankey 다이어그램용 데이터 생성"""

    # 노드: K_small 클러스터들 (0~k_small-1) + K_large 클러스터들 (k_small ~ k_small+k_large-1)
    node_labels = [f"K{k_small}_p{i}" for i in range(k_small)] + [
        f"K{k_large}_p{i}" for i in range(k_large)
    ]
...
def generate_sankey_html(
    ...
    link_colors = [
        (
            f"rgba{tuple(list(int(colors_small[s].split('(')[1].split(',')[0]) for _ in range(3)) + [0.4])}"
            if s < k_small
            else "rgba(180,180,180,0.4)"
        )
        for s in sources
    ]
    # 간단하게 회색 반투명으로
...
def main():
    ...
    parser.add_argument("--k-small", type=int, default=3, help="작은 K값")
    parser.add_argument("--k-large", type=int, default=3, help="큰 K값")
