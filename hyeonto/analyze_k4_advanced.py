...
def analyze_marker_patterns(df: pd.DataFrame, cluster_col: str = "cluster_id"):
    ...
    marker_cols = [c for c in df.columns if "marker" in c.lower() or "hyeonto" in c.lower()]
    ...
    for cluster_id in sorted(df[cluster_col].unique()):
        ...
        # 서종별 분포 계산
        cluster_df["category"] = cluster_df["book_name"].apply(categorize_book_simple)
...
def generate_heatmap_data(results: list, output_path: Path):
    ...
    categories = ["사서(四書)", "삼경(三經)", "사서(史書)", "집부(集部)", "예기(禮記)", "당시(唐詩)", "기타"]
    ...
    output_path.write_text(json.dumps(heatmap_data, ensure_ascii=False, indent=2), encoding="utf-8")
...
<head>
    ...
    <title>{layer_name} K=4 서종 분포 히트맵</title>
...
<body>
    <div class="container">
        <h1>📊 {layer_name} K=4 클러스터별 서종 분포</h1>
...
def analyze_linguistic_characteristics(df: pd.DataFrame, cluster_col: str = "cluster_id"):
    """클러스터별 언어학적 특성 분석"""
    ...
    # 서종 분류 추가
    df["category"] = df["book_name"].apply(categorize_book_simple)
...
def main():
    ...
    print("="*80)
    print("📊 K=4 클러스터 심층 분석 (확장판)")
    print("="*80)
    
    # 1. Sentence Boundary K=4 분석
    sentence_path = reports_dir / "sentence_boundary_k4_full" / "boundary_clusters.csv"
    if sentence_path.exists():
        ...
        generate_heatmap_data(sentence_results, reports_dir / "sentence_boundary_k4_full" / "heatmap_data.json")
        generate_html_heatmap(sentence_results, "문장경계(Sentence)", reports_dir / "sentence_boundary_k4_full" / "k4_heatmap.html")
        analyze_linguistic_characteristics(sentence_df)

    # 2. Phrase Boundary K=4 분석
    phrase_path = reports_dir / "phrase_boundary_k4_full" / "phrase_boundary_clusters.csv"
    if phrase_path.exists():
        ...
        generate_heatmap_data(phrase_results, reports_dir / "phrase_boundary_k4_full" / "heatmap_data.json")
        generate_html_heatmap(phrase_results, "구경계(Phrase)", reports_dir / "phrase_boundary_k4_full" / "k4_heatmap.html")
