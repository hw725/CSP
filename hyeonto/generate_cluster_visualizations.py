...
def generate_joint_embedding(
    df: pd.DataFrame, out_dir: Path, dataset_type: str
) -> None:
    ...
    cluster_col = (
        "parent_cluster_id" if "parent_cluster_id" in df.columns else "cluster_id"
    )
    marker_col = (
        "marker_normalized" if "marker_normalized" in df.columns else "현토마커"
    )
    ...
    cluster_marker_matrix = (
        df.groupby([cluster_col, marker_col]).size().unstack(fill_value=0)
    )
    ...
    mapping_df.to_csv(
        joint_dir / "cluster_marker_matrix.csv", index=False, encoding="utf-8-sig"
    )
    print(f"✅ Saved: {joint_dir / 'cluster_marker_matrix.csv'}")

def generate_marker_distribution(
    df: pd.DataFrame, out_dir: Path, dataset_type: str
) -> None:
    """마커 분포 히트맵 및 엔트로피 분석."""
    ...
    entropy_df.to_csv(
        marker_dir / "cluster_entropy.csv", index=False, encoding="utf-8-sig"
    )
...
def generate_visualization(df: pd.DataFrame, out_dir: Path, dataset_type: str) -> None:
    ...
    if "umap_x" not in df.columns and "tsne_x" not in df.columns:
        ...
        df_viz = df.copy()
        df_viz["viz_x"] = (
            df[cluster_col].map(cluster_map).apply(lambda c: cluster_centers[c, 0])
            + np.random.randn(len(df)) * 0.5
        )
        df_viz["viz_y"] = (
            df[cluster_col].map(cluster_map).apply(lambda c: cluster_centers[c, 1])
            + np.random.randn(len(df)) * 0.5
        )
    else:
        df_viz = df.copy()
    ...
    book_col = (
        "book_name"
        if "book_name" in df_sample.columns
        else ("book" if "book" in df_sample.columns else None)
    )
    ...
    fig = px.scatter(
        ...
        y="viz_y",
        color=cluster_col,
        hover_data=[book_col] if book_col is not None else None,
    ...
    book_col = (
        ...
        else ("book" if "book" in df.columns else None)
    )
    agg_map = {}
    if book_col is not None:
        agg_map[book_col] = lambda x: x.value_counts().index[0]

    cluster_stats = df.groupby(cluster_col).agg(agg_map).reset_index()
    cluster_stats["size"] = df.groupby(cluster_col).size().values
    cluster_stats.to_csv(
        viz_dir / "cluster_summary_stats.csv", index=False, encoding="utf-8-sig"
    )
...
def main() -> int:
    ...
    p.add_argument(
        "--dataset-type", choices=["pa", "sa"], default="pa", help="데이터셋 유형"
    )
    args = p.parse_args()
