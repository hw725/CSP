...
def process_batch_sentences(sentence_batch, device_id=None):
    ...
    try:
        if verbose_mode:
            print(f"🔄 워커 {mp.current_process().pid}: {batch_size}개 문장 배치 시작")
        
        for idx, sentence_data in enumerate(sentence_batch):
            try:
                # 더 자주 진행 상황 출력 (작은 배치에서도)
                if batch_size <= 20:
                    # 작은 배치: 5개마다
                    if (idx + 1) % 5 == 0:
        ...
        if verbose_mode:
            print(f"✅ 워커 {mp.current_process().pid}: 배치 완료 → {len(results)}개 구 ({processing_time:.2f}초)")
...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
    ...
    try:
        if parallel and total_sentences > workers:
            ...
            # 배치 크기를 매우 작게 설정 (핵심 변경사항)
            if total_sentences <= 100:
                batch_size_per_worker = 10  # 100개 이하: 배치당 10개
            elif total_sentences <= 500:
                batch_size_per_worker = 20  # 500개 이하: 배치당 20개
            elif total_sentences <= 1000:
                batch_size_per_worker = 30  # 1000개 이하: 배치당 30개
            else:
                batch_size_per_worker = 50  # 1000개 초과: 배치당 50개 (최대)
            
            # 추가로 워커 수에 따라 더 세분화
            if workers >= 4:
                batch_size_per_worker = min(batch_size_per_worker, 25)  # 워커 많으면 더 작게
            ...
            with mp.Pool(processes=workers, initializer=init_worker, 
                ...
                # 결과 수집 - 타임아웃은 기존 유지, 더 자주 업데이트
                completed_batches = 0
                processed_sentences = 0
                failed_batches = 0
                
                print(f"🔄 배치 처리 시작 (총 {len(sentence_batches)}개 배치)")
                
                for batch_idx, async_result, batch_size in async_results:
                    try:
                        # 타임아웃을 문장당 10초로 증가, 최소 5분 유지
                        timeout_seconds = max(300, batch_size * 10)  # 문장당 10초, 최소 5분
                        
                        # 더 자주 상태 업데이트
                        if batch_idx % 5 == 0 or batch_size > 30:
                            print(f"⏳ 배치 {batch_idx+1}/{len(sentence_batches)} 처리 중... (크기: {batch_size}, 타임아웃: {timeout_seconds}초)")
                        ...
                        # 성공 메시지 - 큰 배치나 주기적으로만 출력
                        if batch_idx % 10 == 0 or batch_size > 30 or completed_batches == len(sentence_batches):
                            print(f"✅ 배치 {batch_idx+1}/{len(sentence_batches)} 완료: {len(batch_results)}개 구 생성 (누적: {completed_batches}개 배치)")
                        
                        logger.info(f"배치 {batch_idx+1}/{len(sentence_batches)} 완료: {len(batch_results)}개 구")
                        
                    except mp.TimeoutError:
                        error_msg = f"배치 {batch_idx+1} 타임아웃 (크기: {batch_size}, 제한: {timeout_seconds}초)"
            ...
            # 최종 결과 출력
            success_rate = ((len(sentence_batches) - failed_batches) / len(sentence_batches)) * 100
            print(f"📊 배치 처리 완료:")
            print(f"   총 배치: {len(sentence_batches)}개")
            print(f"   성공: {len(sentence_batches) - failed_batches}개")
            print(f"   실패: {failed_batches}개")
            print(f"   성공률: {success_rate:.1f}%")
            
            if failed_batches > 0:
                print(f"⚠️  경고: {failed_batches}개 배치 실패")
                logger.warning(f"{failed_batches}개 배치 처리 실패")
