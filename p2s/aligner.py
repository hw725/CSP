...
def safe_source_split(
    tgt_sentences: List[str], src_text: str, tokenizer_func=None, nlp=None
) -> List[str]:
    """무결성 보장 원문 분할"""
    ...
    try:
        # 새 파서들 방식 또는 기본 방식
        if tokenizer_func:
            src_chunks = split_src_by_tgt_units_new_parsers(
                tgt_sentences, src_text, tokenizer_func
            )
        else:
            src_chunks = split_src_by_tgt_units_vice_versa(
                tgt_sentences, src_text, None, tokenizer_func
            )

        if not src_chunks:
            src_chunks = split_source_by_whitespace_and_align(
                src_text, len(tgt_sentences)
            )
...
def verify_paragraph_integrity(
    src_paragraph: str, tgt_paragraph: str, alignments: List[Dict]
) -> bool:
    ...
    if not src_integrity:
        logger.error(
            f"원문 무결성 실패 - 원본: {len(original_src)}자, 결과: {len(aligned_src)}자"
        )
        logger.error(
            f"원본(raw→norm): {raw_original_src[:80]} → {original_src[:80]} ..."
        )
        logger.error(f"결과(raw→norm): {raw_aligned_src[:80]} → {aligned_src[:80]} ...")

    if not tgt_integrity:
        logger.error(
            f"번역문 무결성 실패 - 원본: {len(original_tgt)}자, 결과: {len(aligned_tgt)}자"
        )
        logger.error(
            f"원본(raw→norm): {raw_original_tgt[:80]} → {original_tgt[:80]} ..."
        )
        logger.error(f"결과(raw→norm): {raw_aligned_tgt[:80]} → {aligned_tgt[:80]} ...")

    # 2. 정렬 품질 검증: 빈 원문/번역문 비율 체크
    empty_src_count = sum(
        1 for align in alignments if not align.get("원문", "").strip()
    )
    empty_tgt_count = sum(
        1 for align in alignments if not align.get("번역문", "").strip()
    )
    ...
    if empty_src_ratio > 0.3:
        logger.error(
            f"원문 빈 행 과다: {empty_src_count}/{total_count} ({empty_src_ratio:.1%})"
        )
        alignment_quality = False

    if empty_tgt_ratio > 0.3:
        logger.error(
            f"번역문 빈 행 과다: {empty_tgt_count}/{total_count} ({empty_tgt_ratio:.1%})"
        )
        alignment_quality = False

    return src_integrity and tgt_integrity and alignment_quality

def restore_paragraph_integrity(
    src_paragraph: str, tgt_paragraph: str, alignments: List[Dict]
) -> List[Dict]:
    """문단 무결성 복원 (PA)
    ...
    def _restore_side(original_raw: str, restored_rows: List[Dict], key: str) -> None:
        ...
        if aligned_norm != original_norm:
            ...
            mapped = [
                _translate_pos(b, opcodes, len(aligned_norm), len(original_norm))
                for b in boundaries
            ]
        else:
            mapped = boundaries
...
def split_target_sentences_new_parsers(
    text: str, max_length: int = 150, tokenizer_func=None, use_new_parsers: bool = True
) -> List[str]:
    ...
    if use_new_parsers:
        try:
            ...
            new_parser_sentences = smart_sentence_split(
                text, is_source=False
            )  # 번역문이므로 is_source=False
        ...
        sentences = split_target_sentences_advanced(
            text, max_length, splitter="punctuation"
        )
...
def split_long_sentence_with_tokenizer(
    sentence: str, max_length: int, tokenizer_func
) -> List[str]:
...
def get_embedder_function(
    ...
    if embedder_name == "bge":
        try:
            ...
            def enhanced_embed_func(
                texts, use_multi_vector=True, batch_size_override=None, **kwargs
            ):
                actual_batch_size = (
                    batch_size_override if batch_size_override else batch_size
                )
        ...
        try:
            ...
            print(
                f"✅ OpenAI 임베더 초기화 성공 (max_workers={max_workers}, batch_size={batch_size})"
            )
...
def split_src_by_tgt_units_new_parsers(
    tgt_sentences: List[str], src_text: str, tokenizer_func=None
) -> List[str]:
...
def _strip_brackets_from_text(text: str):
    ...
    if insertions:
        logger.info(
            f"🔥 Strip brackets: {len(insertions)} brackets removed, working_src len {len(text)} → {len(working)}"
        )
...
def _adjust_boundaries_with_llm_and_morphology(
    ...
    try:
        ...
        if len(dp_segments) != target_count and refine_boundaries_with_llm:
            logger.info(
                f"🔧 LLM으로 세그먼트 개수 조정: {len(dp_segments)}개 → {target_count}개"
            )
                ...
                for i, segment in enumerate(dp_segments):
                    ...
                    if i < len(dp_segments) - 1:
                        ...
                        nearest_boundary = min(
                            ec_ef_positions, key=lambda pos: abs(pos - segment_end)
                        )

                        # 경계가 너무 멀리 이동하지 않도록 제한 (±10글자)
                        if abs(nearest_boundary - segment_end) <= 10:
                            # 새 경계로 텍스트 재분할
                            adjusted_segment = src_text[
                                current_pos:nearest_boundary
                            ].strip()
                            adjusted_segments.append(adjusted_segment)
                            current_pos = nearest_boundary
                            logger.debug(
                                f"  경계 조정: {segment_end} → {nearest_boundary}"
                            )
                            continue
...
def _restore_brackets_to_segments(
    segments: List[str], insertions: List[Tuple[int, str]]
) -> List[str]:
    ...
    logger.info(
        f"🔥 Restore brackets: {len(insertions)} brackets to restore into {len(segments)} segments"
    )
    cumulative = [0]
    for ch in segments:
    ...
    for pos, content in insertions:
        idx = 0
        while idx < len(segments) and not (
            cumulative[idx] <= pos <= cumulative[idx + 1]
        ):
            idx += 1
        if idx >= len(segments):
...
def split_source_by_semantic_boundaries(
    src_text: str,
    target_count: int = None,
    embed_func=None,
    semantic_window: int = 120,
    tgt_sentences: List[str] = None,
) -> List[str]:
    """
    ...
    logger.info(
        f"🔍 원문 의미 분할: {len(src_text)}글자 → 목표 {target_count}개 문장 (DP 메인 + 미세 조정)"
    )
    ...
    if num_units < target_count:
        ...
        logger.warning(
            f"⚠️ 어절 수({num_units}) < 목표 문장 수({target_count}), 어절 단위로 반환"
        )
        return src_units
    ...
    if tgt_sentences and embed_func:
        try:
            logger.info(
                f"🎯 DP 기반 원문 분할 시작 (번역문 {len(tgt_sentences)}개 참조)"
            )
            ...
            if dp_segments and len(dp_segments) > 0:
                ...
                if original_flat == dp_flat:
                    ...
                    adjusted_segments = _adjust_boundaries_with_llm_and_morphology(
                        dp_segments, src_text, src_units, tgt_sentences, target_count
                    )

                    if adjusted_segments:
                        logger.info(
                            f"✅ 경계 미세 조정 완료: {len(adjusted_segments)}개"
                        )
                        return adjusted_segments
                    else:
    ...
    try:
        ...
        if SUPAR_AVAILABLE:
            ...
            if boundaries and len(boundaries) > 0:
                logger.info(
                    f"✅ SuPar 경계: {len(boundaries)}개 → {sorted(boundaries)}"
                )
                ...
                if len(segments) > 1:
                    ...
                    if (
                        len(segments) != target_count
                        and tgt_sentences
                        and refine_boundaries_with_llm
                    ):
                        try:
                            adjusted = _adjust_source_segments_to_target(
                                segments,
                            ...
                            if adjusted and len(adjusted) == target_count:
                                logger.info(
                                    f"✅ SuPar+LLM 조정: {len(segments)}개 → {target_count}개"
                                )
                                return adjusted
                        except Exception as e:
    ...
    try:
        ...
        cand = (
            split_target_sentences_advanced(
                src_text, max_length=400, splitter="punctuation"
            )
            or []
        )
        offset = 0
    ...
    if candidates:
        unique_candidates = sorted(
            {pos: tag for pos, tag in candidates if 0 < pos < len(src_text)}.items()
        )
        ...
        for pos, tag in unique_candidates:
            ...
            morph = (
                1.0 if tag.startswith("EF") else 0.6 if tag.startswith("EC") else 0.7
            )
            score = sem + 0.2 * morph
            scored.append((score, pos))
        scored.sort(key=lambda x: x[0], reverse=True)
        choose = (
            sorted([pos for _, pos in scored[: max(0, target_count - 1)]])
            if target_count
            else [pos for _, pos in scored]
        )
        if choose:
            ...
            if len(segs) > 0:
                segments = segs
                logger.info(
                    f"📍 의미+형태 결합 분할: {len(segments)}개 (후보 {len(candidates)}개)"
                )
    else:
    ...
    def merge_smallest_until(segs: List[str], k: int) -> List[str]:
        while len(segs) > k and len(segs) > 1:
            best_idx = min(
                range(len(segs) - 1), key=lambda i: len(segs[i]) + len(segs[i + 1])
            )
...
def _split_source_with_llm_and_target(
    src_text: str, src_units: List[str], tgt_sentences: List[str], target_count: int
) -> List[str]:
    """
...
{{"segments": ["세그먼트1", "세그먼트2", ...]}}"""
    ...
    try:
        ...
        if not isinstance(segments, list) or len(segments) != target_count:
            logger.warning(
                f"LLM 원문 분할: 잘못된 개수 반환 ({len(segments)} != {target_count})"
            )
            return None
...
{{"segments": ["세그먼트1", "세그먼트2", ...]}}"""
    ...
    try:
        ...
        if not isinstance(adjusted, list) or len(adjusted) != target_count:
            logger.warning(
                f"LLM 원문 조정: 잘못된 개수 반환 ({len(adjusted)} != {target_count})"
            )
            return None
...
def align_sentences_with_optimal_matching(
    ...
    if M > 1 and N == 1:
        ...
        # 번역문도 의미 경계로 재분할
        semantic_tgt_sentences = split_target_sentences_advanced(
            tgt_text, splitter="punctuation"
        )
            ...
            for j in range(N):
                if src_embs is not None and tgt_embs is not None:
                    ...
                    if src_norm > 0 and tgt_norm > 0:
                        sim_matrix[i, j] = np.dot(src_embs[i], tgt_embs[j]) / (
                            src_norm * tgt_norm
                        )
                else:
                    src_len = len(src_sentences[i].split())
                    tgt_len = len(tgt_sentences[j].split())
                    sim_matrix[i, j] = 1.0 - abs(src_len - tgt_len) / max(
                        src_len, tgt_len, 1
                    )
    ...
    if embed_func and M > 1 and N > 1 and alignments:
        sims = [
            ...
            for a in alignments
            if isinstance(a.get("similarity", None), (int, float))
        ]
        if sims:
            ...
            if avg_sim < 0.25:  # 임계값 완화: 0.2 → 0.25
                ...
                try:
                    ...
                    original_src_normalized = (
                        src_text_full.replace(" ", "")
                        .replace("\n", "")
                        .replace("\t", "")
                    )
                    logger.info(
                        f"  원문 재분할: {len(src_text_full)}자 → {N}개 세그먼트"
                    )
                    reseg = segment_src_by_tgt_similarity(
                        src_text_full,
                        tgt_sentences,
                        embed_func=embed_func,
                        tokenizer_func=tokenizer_func,
                    ...
                    if reseg and len(reseg) == N:
                        ...
                        if not integrity_ok:
                            ...
                            logger.warning(
                                f"⚠️ 재분할 무결성 경고: 원본 {len(original_src_normalized)}자 != 재분할 {len(reseg_normalized)}자 (차이: {diff:+d}자)"
                            )
                        ...
                        if non_empty < N * 0.5:  # 절반 이상 비어있으면 무효
                            logger.warning(
                                f"⚠️ 재분할 무효 (빈 세그먼트 과다: {non_empty}/{N})"
                            )
                        else:
                            ...
                            for i in range(N):
                                ...
                                denom = (
                                    np.linalg.norm(svec) * np.linalg.norm(tvec) + 1e-8
                                )
                                sim_val = (
                                    float(np.dot(svec, tvec) / denom)
                                    if denom > 0
                                    else 0.0
                                )
                                new_align.append(
                            ...
                            if new_align:
                                new_avg = float(
                                    np.mean([a["similarity"] for a in new_align])
                                )
                                # 무결성 OK이거나, 유사도가 크게 개선되면 적용
                                if integrity_ok and (
                                    new_avg > avg_sim or avg_sim < 0.15
                                ):
                                    alignments = new_align
                                    logger.info(
                                        f"✅ 저유사도 폴백 적용: 평균 유사도 {avg_sim:.3f} → {new_avg:.3f} (무결성 보장)"
                                    )
                                elif not integrity_ok and (
                                    new_avg > avg_sim * 1.5 or avg_sim < 0.10
                                ):
                                    # 무결성 문제 있지만 유사도가 1.5배 이상 개선되거나 원래 매우 나쁘면 적용
                                    alignments = new_align
                                    logger.warning(
                                        f"⚠️ 저유사도 폴백 적용 (무결성 경고 무시): 평균 유사도 {avg_sim:.3f} → {new_avg:.3f}"
                                    )
                                else:
                                    logger.info(
                                        f"⚠️ 폴백 결과 나쁨 ({new_avg:.3f}) 또는 무결성 문제, 원본 유지"
                                    )
                    else:
                        logger.warning(
                            f"⚠️ 재분할 실패: 예상 {N}개, 실제 {len(reseg) if reseg else 0}개"
                        )
                except Exception as e:
                ...
                logger.info(
                    f"✅ 유사도 정상 (avg_sim={avg_sim:.3f} >= 0.25), 폴백 불필요"
                )

    logger.info(f"✅ 매칭 완료: {len(alignments)}개 쌍")
    return alignments

def segment_src_by_tgt_similarity(
    src_text: str, tgt_sentences: List[str], embed_func=None, tokenizer_func=None
) -> List[str]:
    """
    ...
    if not embed_func:
        ...
        for s in range(S - 1):
            ...
            if (
                idx < next_idx
                and idx < len(word_spans)
                and next_idx - 1 < len(word_spans)
            ):
                start_char = word_spans[idx][0]
                # trailing whitespace까지 포함하여 원문 간격 보존
                end_char = (
                    word_spans[next_idx][0]
                    if next_idx < len(word_spans)
                    else len(working_src)
                )
    ...
    try:
        ...
        temp_src_sentences = split_func(
            working_src, max_length=200, splitter="punctuation"
        )
    ...
    try:
        ...
        logger.info(
            f"📊 임베딩: 원문어절 {len(token_embs)}, 번역문문장 {len(tgt_sent_embs)}, 원문문장 {len(src_sent_embs)}"
        )
    except Exception as e:
        ...
        try:
            ...
            segments = (
                split_target_sentences_advanced(
                    working_src, max_length=400, splitter="punctuation"
                )
                or []
            )
            ...
            def split_longest(segs):
                while len(segs) < S:
                    idx2 = max(
                        range(len(segs)), key=lambda i: len(segs[i]) if segs[i] else 0
                    )
                    ...
                    if analyzed and analyzed[0]:
                        ...
                        for token in tokens:
                            ...
                            if tag == "EF":  # 종결어미 → 확실한 문장 경계
                                ...
                                logger.debug(
                                    f"  어절 {word_idx} ({word}): EF 종결어미 → bonus +0.20"
                                )
                                break
                                ...
                                if word_idx > 0 and word_idx < W - 1:
                                    ...
                                    logger.debug(
                                        f"  어절 {word_idx} ({word}): EC 연결어미, 좌우 sim={sim:.3f} → bonus +{boundary_score:.3f}"
                                    )
                                else:
                                    tokenizer_boundary_bonus[word_idx] = 0.08
                                    logger.debug(
                                        f"  어절 {word_idx} ({word}): EC 연결어미 (경계) → bonus +0.08"
                                    )
                                break
                except Exception as e_inner:
                    logger.debug(
                        f"  어절 {word_idx} ({word}): kiwipiepy 분석 실패: {e_inner}"
                    )
            ...
            # 2️⃣ 순수 한문 어절 → siku-bert로 구두점 또는 구조적 경계 탐지
                ...
                if siku_tokenizer:
                    try:
                        # siku-bert 토크나이저 적용
                        tokens = (
                            siku_tokenizer(word) if callable(siku_tokenizer) else []
                        )
                        # 여러 토큰으로 분할되거나 특수 토큰이 있으면 구조적 경계 가능
                        if tokens and len(tokens) > 1:
                            tokenizer_boundary_bonus[word_idx] = 0.06
                            logger.debug(
                                f"  어절 {word_idx} ({word}): 한문 다중 토큰({len(tokens)}개) → bonus +0.06"
                            )
                    except Exception as e_inner:
                        logger.debug(
                            f"  어절 {word_idx} ({word}): siku-bert 분석 실패: {e_inner}"
                        )
    ...
    if tokenizer_boundary_bonus:
        logger.info(
            f"📊 형태소 경계 힌트: {len(tokenizer_boundary_bonus)}개 어절 (siku-bert + kiwipiepy EC/EF 동적 평가)"
        )
    else:
    ...
    try:
        ...
        logger.debug(
            f"📊 파서 설정: SUPAR_AVAILABLE={SUPAR_AVAILABLE}, STANZA_AVAILABLE={STANZA_AVAILABLE}"
        )

        # 원문 경계 정보 (SuPar-Kanbun)
        if SUPAR_AVAILABLE:
            try:
                src_unit_boundaries = get_chinese_unit_boundary_indices_supar(
                    working_src, words
                )
                ...
                if src_unit_boundaries:
                    ...
                    for word_idx, word in enumerate(words):
                        ...
                        if any(
                            abs(cumulative_len - b) <= 2 for b in src_unit_boundaries
                        ):
                            parser_boundary_bonus[word_idx] = 0.1
                            logger.debug(
                                f"    어절 {word_idx} ({word}): SuPar 경계 힌트 (+0.1)"
                            )
            except Exception as e:
        ...
        if STANZA_AVAILABLE:
            try:
                tgt_clause_boundaries = get_korean_clause_boundaries_stanza(
                    tgt_paragraph, mode="default"
                )
    ...
    try:
        src_tgt_sim_matrix = np.zeros(
            (len(src_sentences), len(tgt_sentences)), dtype=float
        )
        for i, src_sent_emb in enumerate(src_sent_embs):
            for j, tgt_sent_emb in enumerate(tgt_sent_embs):
                try:
                    sim = float(
                        np.dot(src_sent_emb, tgt_sent_emb)
                        / (
                            np.linalg.norm(src_sent_emb) * np.linalg.norm(tgt_sent_emb)
                            + 1e-8
                        )
                    )
    ...
    for i in range(1, W + 1):
        try:
            ...
            # 번역문 유사도
            sim_tgt = float(
                np.dot(span_vec, tgt_vec)
                / (np.linalg.norm(span_vec) + 1e-8)
                / (np.linalg.norm(tgt_vec) + 1e-8)
            )
            ...
            if use_bidirectional:
                ...
                src_sims = [
                    float(
                        np.dot(span_vec, src_emb)
                        / (np.linalg.norm(span_vec) + 1e-8)
                        / (np.linalg.norm(src_emb) + 1e-8)
                    )
    ...
    for j in range(1, S):
        ...
        for i in range(j + 1, W + 1):
            ...
            for k in range(k_min, i):
                ...
                denom_tgt = (np.linalg.norm(span_vec) + 1e-8) * (
                    np.linalg.norm(tgt_sent_vec) + 1e-8
                )
                    ...
                    try:
                        ...
                        for src_sent_emb in src_sent_embs:
                            try:
                                ...
                                denom_src = (np.linalg.norm(span_vec) + 1e-8) * (
                                    np.linalg.norm(src_sent_emb) + 1e-8
                                )
            ...
            for w_idx in range(W):
                ...
                for s_idx in range(S):
                    try:
                        ...
                        sim = float(
                            np.dot(word_vec, sent_vec)
                            / (np.linalg.norm(word_vec) + 1e-8)
                            / (np.linalg.norm(sent_vec) + 1e-8)
                        )
                        if sim > best_sim:
                            best_sim = sim
                            best_sent_idx = s_idx
...
def process_paragraph_alignment(
    ...
    try:
        ...
        embed_func = get_embedder_function(
            embedder_name, device=device, max_workers=max_workers, batch_size=batch_size
        )
        tokenizer_func = get_tokenizer_function(tokenizer_name)

        # 번역문 분할 (구두점 기반)
        tgt_sentences = split_target_sentences_advanced(
            tgt_paragraph, max_length=max_length, splitter="punctuation"
        )
        ...
        print(
            f"🔄 원문 {len(src_sentences)}개 문장 ↔ 번역문 {len(tgt_sentences)}개 문장 정렬 중..."
        )
        logger.info(
            f"🔄 원문 {len(src_sentences)}개 문장 ↔ 번역문 {len(tgt_sentences)}개 문장 정렬 중..."
        )
        ...
        if not embed_func or len(src_sentences) == 0 or len(tgt_sentences) == 0:
            logger.error(
                f"문장 정렬 불가: embed_func={bool(embed_func)}, src={len(src_sentences)}, tgt={len(tgt_sentences)}"
            )
...
def process_paragraph_file(
    ...
    **kwargs,
    ...
    original_tgt_all = (
        "".join(df["번역문"].fillna("")) if "번역문" in df.columns else ""
    )
    ...
    # 원문 무결성 검증
    src_valid, src_msg = integrity_manager.verify_integrity(
        output_src_all, f"{file_id}_src_all"
    )
    if not src_valid:
    ...
    tgt_valid, tgt_msg = integrity_manager.verify_integrity(
        output_tgt_all_after_src, f"{file_id}_tgt_all"
    )
    ...
    final_src_valid, _ = integrity_manager.verify_integrity(
        final_src_check, f"{file_id}_src_all"
    )
    final_tgt_valid, _ = integrity_manager.verify_integrity(
        final_tgt_check, f"{file_id}_tgt_all"
    )
