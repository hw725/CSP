...
def merge_and_save(source_data, translation_data, output_excel_path, book_name):
    ...
    # 모든 s_id와 w_id 수집
    all_s_ids = sorted(
        set(list(source_data.keys()) + list(translation_data.keys())),
    ...
    for s_id in all_s_ids:
        ...
        all_w_ids = sorted(
            source_w_ids | trans_w_ids, key=lambda x: int(x) if x.isdigit() else 0
        )
...
def process_xml_pairs(source_dir, output_base_dir):
    ...
    for source_xml in source_files:
        ...
        try:
            ...
            # Excel로 저장
            row_count = merge_and_save(
                source_data, translation_data, output_excel, book_name
            )
            ...
            if src_w_count != row_count or tgt_w_count != row_count:
                ...
                if only_in_src:
                    ...
                    for s_id, w_id in sorted(list(only_in_src)[:10]):  # 최대 10개
                        ...
                        log_entries.append(
                            f"      - s_id={s_id}, w_id={w_id}: '{src_text}'"
                        )
                ...
                if only_in_tgt:
                    ...
                    for s_id, w_id in sorted(list(only_in_tgt)[:10]):  # 최대 10개
                        ...
                        log_entries.append(
                            f"      - s_id={s_id}, w_id={w_id}: '{tgt_text}'"
                        )
                ...
                if duplicates:
                    ...
                    for wid, cnt in sorted(
                        list(duplicates.items())[:10], key=lambda x: -x[1]
                    ):
                        # 어느 s_id들에서 중복되는지 찾기
                        s_ids_with_wid = [
                            s_id for s_id in source_data if wid in source_data[s_id]
                        ]
                        log_entries.append(
                            f"      - w_id={wid}: {cnt}번 등장, s_id={s_ids_with_wid[:5]}"
                        )
                ...
                tgt_duplicates = {
                    wid: cnt for wid, cnt in tgt_w_id_counts.items() if cnt > 1
                }

                if tgt_duplicates:
                    log_entries.append(
                        f"   ⚠️ 번역문 중복 w_id ({len(tgt_duplicates)}개):"
                    )
                    for wid, cnt in sorted(
                        list(tgt_duplicates.items())[:10], key=lambda x: -x[1]
                    ):
                        s_ids_with_wid = [
                            s_id
                            for s_id in translation_data
                            if wid in translation_data[s_id]
                        ]
                        log_entries.append(
                            f"      - w_id={wid}: {cnt}번 등장, s_id={s_ids_with_wid[:5]}"
                        )
                ...
                print(
                    f"  ⚠️ 불일치: 원문 {src_w_count}개, 번역문 {tgt_w_count}개, XLSX {row_count}행"
                )
