...
def analyze_text_integrity(
    gt_text: str, pred_text: str, text_name: str = "텍스트"
) -> Dict[str, Any]:
    """
    ...
    only_space_add = (
        all(add["type"] == "공백" for add in additions) if additions else True
    )
...
def verify_global_integrity(
    ...
    # 2. 출력 파일 전체 텍스트 복원
    pred_sources = "".join(
        str(row.get(source_col, "")) for _, row in result_df.iterrows()
    )
    pred_targets = "".join(
        str(row.get(target_col, "")) for _, row in result_df.iterrows()
    )
            ...
            if src_analysis["only_space_loss"] and src_analysis["delta"] > 0:
                ...
                if src_analysis["losses"]:
                    for loss in src_analysis["losses"][:3]:
                        print(
                            f"    손실 [{loss['type']}] '{loss['lost_text']}' (위치: {loss['position']})"
                        )
                ...
                if src_analysis["additions"]:
                    for add in src_analysis["additions"][:3]:
                        print(
                            f"    추가 [{add['type']}] '{add['added_text']}' (위치: {add['position']})"
                        )
    ...
    analysis = {
        ...
        "total_additions": len(src_analysis["additions"])
        + len(tgt_analysis["additions"]),
    }
