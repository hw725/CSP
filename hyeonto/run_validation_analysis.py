...
def run_retrospective_aspect_test(df: pd.DataFrame, n_permutations: int = 1000):
    ...
    df_copy['book_genre'] = df_copy['book'].apply(classify_book_genre)
    ...
    inverse_interpretation = "? 가중치와 무관하게 역사서 집중 유지" if inverse_maintains else "?? 가중치에 민감"
    ...
    for genre in ['역사서', '문집', '사서', '삼경', '예학']:
        ...
        genre_count = df_copy[(df_copy['book_genre'] == genre if genre == '역사서' 
                              ...
                              else df_copy['book'].apply(lambda x: genre in classify_book(x))) 
...
def main():
    ...
    hypothesis_results = {
        ...
        'final_verdict': '? 모든 검증 통과: 사서 중심성 + 회상상 역사서 집중 확인' if (all_passed and retro_passed) else '?? 일부 검증 실패',
