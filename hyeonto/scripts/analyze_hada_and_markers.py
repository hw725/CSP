...
def analyze_hada_sentence():
    ...
    table = np.array(
        [
            ...
            [
                non_history["is_hada"].sum(),
                len(non_history) - non_history["is_hada"].sum(),
            ],
        ]
    )
    ...
    print(
        f"결론: {'H₀ 기각 ✅ (역사서에서 유의미하게 높음)' if p_value < 0.05 else 'H₀ 기각 실패'}"
    )
...
def analyze_classified_markers():
    ...
    with open(
        RESULTS_DIR / "classified_markers_phrase.json", "w", encoding="utf-8"
    ) as f:
