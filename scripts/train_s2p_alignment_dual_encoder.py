#!/usr/bin/env python3
"""Train dual-encoder alignment model for S2P using TRAIN-ONLY CSV.

핵심 원칙:
- 학습 데이터는 train만 사용 (val/test 사용 금지)
- 원문(src)과 번역문(tgt) 쌍을 동시에 학습
- Hard Negative: 같은 배치 내 다른 쌍을 negative로 사용
- 출력: models/dual_encoder_alignment_s2p.pt (legacy: dual_encoder_alignment_sa.pt)

PA 버전(train_alignment_dual_encoder_trainonly.py)을 S2P에 맞게 수정.
"""
...
class SaTrainCsvDataset(Dataset):
    ...
    def __init__(
        ...
        print(f"📂 Loaded {len(self.src_texts)} S2P pairs from {csv_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Train S2P Dual-Encoder Alignment")
    parser.add_argument(
        "--train-csv",
        type=str,
        default="datasets/s2p/train.csv",
        help="학습용 S2P CSV 경로",
    )
    ...
    parser.add_argument(
        "--out",
        type=str,
        default="models/dual_encoder_alignment_s2p.pt",
        help="출력 모델 경로",
    )
    ...
    print("✅ S2P Dual-Encoder Alignment 학습 완료!")
    return 0
