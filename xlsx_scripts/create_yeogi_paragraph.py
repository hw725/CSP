"""
예기집설대전 문장병렬 Excel 파일을 문단식별자 기준으로 묶어서 문단병렬 파일 생성
컬럼: 문단식별자, 원문, 번역문
NaN 값은 그대로 보존
"""
...
def create_paragraph_parallel(output_base_dir):
    """단락 ID 기반 책들의 문장병렬 파일을 문단병렬로 변환 (NaN 보존)"""
    
    output_base_dir = Path(output_base_dir)
    
    # 단락 ID 기반 책들의 문장병렬 Excel 파일 찾기
    target_books = ['예기집설대전1', '예기집설대전2', '당시삼백수1', '당시삼백수2', '당시삼백수3']
    excel_files = []
    for book in target_books:
        files = list(output_base_dir.glob(f'*/{book}_문장병렬.xlsx'))
        excel_files.extend(files)
    
    print(f"단락 ID 기반 책 문단병렬 생성 중...")
    print(f"총 {len(excel_files)}개 파일 처리\n")
