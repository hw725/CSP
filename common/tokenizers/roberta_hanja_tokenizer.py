...
class RobertaKoreanHanjaTokenizer:
    ...
    def _initialize(self):
        ...
        try:
            ...
            logger.info(f"RoBERTa Korean-Hanja 모델 로드 완료: {self.model_name} on {self.device}")
...
def get_roberta_hanja_tokenizer(model_name: str = "klue/roberta-large", device: str = "auto") -> RobertaKoreanHanjaTokenizer:
    """RoBERTa Korean-Hanja 토크나이저 싱글톤 인스턴스 반환"""
    global _roberta_hanja_tokenizers
    
    key = f"{model_name}_{device}"
    if key not in _roberta_hanja_tokenizers:
        _roberta_hanja_tokenizers[key] = RobertaKoreanHanjaTokenizer(model_name=model_name, device=device)
    
    return _roberta_hanja_tokenizers[key]

def roberta_extract_hanja(text: str, model_name: str = "klue/roberta-large") -> List[Dict[str, Any]]:
...
def roberta_tokenize_hanja(hanja_text: str, model_name: str = "klue/roberta-large") -> List[str]:
