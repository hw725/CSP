...
class UnifiedProgressManager:
    ...
    def start_progress(
        self, total: int, description: str = "🔄 처리 중", **kwargs
    ) -> None:
        ...
        if not self.line_mode:
            ...
            self.progress_bar = tqdm(total=total, desc=description, **default_kwargs)
        else:
            # 라인 모드: 초기 상태 한 줄 출력
    ...
    def update(self, n: int = 1, **postfix) -> None:
        ...
        if self.line_mode:
            # 라인 모드: 간단한 상태 줄 출력
            info_str = (
                " ".join([f"{k}={v}" for k, v in {**self.task_info, **postfix}.items()])
                if (self.task_info or postfix)
                else ""
            )
            print(
                f"{self.current_task}: {self.completed_steps}/{self.total_steps} {info_str}"
            )
...
def start_unified_progress(
    total: int, description: str = "🔄 처리 중", **kwargs
) -> None:
    """통합 진행률 시작 (편의 함수)"""
