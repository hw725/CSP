...
def normalize_marker(marker: str) -> str:
    ...
    if marker.endswith("로다"):
        ...
        marker.endswith("라")
        and not marker.endswith("니라")
        and not marker.endswith("더라")
        and not marker.endswith("러라")
    ):
        return "라"
    return marker
...
def append_to_csv(df: pd.DataFrame, filepath: Path):
    ...
    df.to_csv(
        filepath, mode="a", header=write_header, index=False, encoding="utf-8-sig"
    )

async def process_batches_parallel(
    data_df, translations_col, analyze_func, marker_type, level, csv_path, desc
):
    """배치들을 병렬로 처리"""
...
async def main_async():
    ...
    contingency = [
        [roda_positive, len(roda_results) - roda_positive],
    ...
    print(
        f"         대조군: {control_positive}/{len(control_results)} ({control_positive/len(control_results)*100:.1f}%)"
    )
    ...
    stats_results.append(
        {
            "level": "Level 1",
            "marker": "로다",
            "n_target": int(len(roda_results)),
            "n_control": int(len(control_results)),
            "target_positive": roda_positive,
    ...
    contingency2 = [
        [nira_positive, len(nira_results) - nira_positive],
    ...
    print(
        f"         라: {ra_positive}/{len(ra_results)} ({ra_positive/len(ra_results)*100:.1f}%)"
    )
    ...
    stats_results.append(
        {
            "level": "Level 2",
            "marker": "니라 vs 라",
            "n_nira": int(len(nira_results)),
            "n_ra": int(len(ra_results)),
            "nira_positive": nira_positive,
