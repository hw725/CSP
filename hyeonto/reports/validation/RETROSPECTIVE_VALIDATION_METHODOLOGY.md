...
df['has_reo'] = df['marker_normalized'].apply(
    lambda x: has_marker(x, [r'러(?=[ㄱ-ㅎㅏ-ㅣ가-힣])', r'려(?=[ㄱ-ㅎㅏ-ㅣ가-힣])'])
)
...
| **H_inv (반대가설)** | SUPPORT | 회상 23.3% < 단순과거 63.3% | ⚠️ LLM 범주 분류 기준 검토 필요 |
...
> ⚠️ **본 결과는 파일럿 테스트입니다. 향후 더 큰 샘플로 재검증 예정:**
>
> 1. **전수조사**: -러- 전체 5,094건 + 대조군 5,000건 (Ollama 클라우드 사용량 제한 고려, 분할 수행)
> 2. **학술적 정의 확보**: 고전 국어학 문헌에서 '회상상'의 정확한 정의 확인
> 3. **프롬프트 개선**: 학술적 정의에 기반한 판단 기준 정교화
> 4. **OpenAI API 도입**: 전수조사 효율화 (병렬 처리)
