...
try:
    ...
    logger.warning(f"SA 임베더를 사용할 수 없습니다. 순차적 정렬만 지원됩니다. ({e})")
...
class ParagraphAligner:
    ...
    def _verify_and_repair_integrity(self, source_text: str, target_sentences: List[str], aligned_pairs: List[Dict]) -> Tuple[List[Dict], bool]:
        ...
        try:
            ...
            original_src_norm = normalize_text(source_text)
            aligned_src_norm = normalize_text(aligned_src_text)
            
            original_tgt_norm = normalize_text(" ".join(target_sentences))
            aligned_tgt_norm = normalize_text(aligned_tgt_text)
...
def process_paragraph_file(
    ...
    try:
        ...
        if results:
            ...
            io_manager.write_excel(result_df, output_file)
