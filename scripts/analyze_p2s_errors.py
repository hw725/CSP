...
def analyze_selection_stage(
    trace_records: List[dict], results: Dict[str, dict], gold: Dict[str, dict]
):
...
def print_family_stats(family_stats: dict):
    ...
    print("\n=== Family별 성능 분석 ===")
    print(
        f"{'Family':<20} {'Count':<8} {'Success':<8} {'Fail':<8} {'Success%':<10} {'Avg Score':<10}"
    )
    print("-" * 80)
    ...
    for family in sorted(family_stats.keys()):
        ...
        print(
            f"{family:<20} {count:<8} {success:<8} {fail:<8} {success_rate:<10.2f} {avg_score:<10.4f}"
        )
...
def print_candidate_quality(analysis: dict):
    ...
    print(f"총 케이스: {total}")
    print(
        f"단일 후보 강제: {analysis['single_candidate_forced']} ({analysis['single_candidate_forced']/total*100:.1f}%)"
    )
    print(f"\n점수 차이 분포:")
    print(
        f"  작음 (<0.05):    {analysis['score_margin_small']} ({analysis['score_margin_small']/total*100:.1f}%)"
    )
    print(
        f"  중간 (0.05~0.15): {analysis['score_margin_medium']} ({analysis['score_margin_medium']/total*100:.1f}%)"
    )
    print(
        f"  큼 (>=0.15):     {analysis['score_margin_large']} ({analysis['score_margin_large']/total*100:.1f}%)"
    )
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python analyze_p2s_errors.py <run_dir>")
        print(
            "  run_dir: 실험 결과가 있는 디렉토리 (예: test_results/multitest_seed1_10_markerbonusA_skipfixA/20260106_074514)"
        )
        sys.exit(1)
    ...
    for seed_num in [1, 2, 3]:  # 일단 3개 seed만
        trace_file = run_dir / f"p2s_trace_seed{seed_num}.jsonl"
        result_file = run_dir / f"p2s_output_n100_seed{seed_num}.csv"
        gold_file = run_dir / f"p2s_gold_subset_n100_seed{seed_num}.csv"
