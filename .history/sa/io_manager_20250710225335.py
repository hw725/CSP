...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
    ...
    try:
        if parallel and total_sentences > workers:
            ...
            # 배치 크기를 더 작게 조정 (문제 해결)
            max_batch_size = 100  # 기존보다 훨씬 작게
            batch_size_per_worker = min(max_batch_size, max(1, total_sentences // (workers * 2)))
            sentence_batches = []
            ...
            print(f"📦 배치 분할: {len(sentence_batches)}개 배치 (배치당 최대 {batch_size_per_worker}개)")
            ...
            with mp.Pool(processes=workers, initializer=init_worker, 
                ...
                for batch_idx, async_result, batch_size in async_results:
                    try:
                        # 타임아웃을 배치 크기에 비례해서 설정 (문제 해결)
                        timeout_seconds = max(300, batch_size * 5)  # 문장당 5초, 최소 5분
                        
                        print(f"⏳ 배치 {batch_idx+1}/{len(sentence_batches)} 처리 중... (타임아웃: {timeout_seconds}초)")
                        ...
                        # 상태 메시지 업데이트
                        progress_bar.set_description(f"✅ 배치 {completed_batches}/{len(sentence_batches)} 완료")
                        
                        print(f"✅ 배치 {batch_idx+1}/{len(sentence_batches)} 완료: {len(batch_results)}개 구 생성")
                        logger.info(f"배치 {batch_idx+1}/{len(sentence_batches)} 완료: {len(batch_results)}개 구")
                        
                    except mp.TimeoutError:
                        error_msg = f"배치 {batch_idx+1} 타임아웃 ({timeout_seconds}초 초과)"
                        ...
                        error_msg = f"배치 {batch_idx+1} 처리 오류: {e}"
                        logger.error(error_msg)
                        print(f"❌ {error_msg}")
