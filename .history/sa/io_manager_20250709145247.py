...
def init_worker(device_id=None, embedder_name='bge', openai_model=None, openai_api_key=None):
    ...
    try:
        ...
        worker_embed_func = get_embedder(embedder_name, device_id=device_id, openai_model=openai_model, openai_api_key=openai_api_key)
        ...
        if verbose_mode:
            print(f"워커 {mp.current_process().pid}: 임베더 초기화 완료 ({embedder_name})")
...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
                batch_size: int = 20, device_ids=None, verbose: bool = False, embedder_name: str = 'bge', openai_model: str = None, openai_api_key: str = None):
    """개선된 병렬 처리"""
    ...
    if parallel and len(sentence_data_list) > workers:
        ...
        # 프로세스 풀로 배치 처리
        with mp.Pool(processes=workers, initializer=init_worker, initargs=(device_ids[0], embedder_name, openai_model, openai_api_key)) as pool:
            try:
                async_results = []
