...
class IntegratedAnalyzer:
    ...
    def __init__(
        self, csv_file: str = None, json_file: str = "cumulative_analysis_report.json"
    ):
        ...
        if csv_file is None:
            if Path("cumulative_analysis_results_manual.csv").exists():
                ...
                print(
                    "📝 수동 편집된 CSV 파일을 사용합니다: cumulative_analysis_results_manual.csv"
                )
            else:
    ...
    def _prepare_clustering_data_from_csv(self):
        ...
        for i in range(min(3, len(self.df))):
            row = self.df.iloc[i]
            print(
                f"📊 Row {i+1}: 책명={row.get('책명', '없음')}, 시대={row.get('시대', '없음')}, 4부분류={row.get('4부분류', '없음')}"
            )
        ...
        for _, row in self.df.iterrows():
            ...
            dynasty = (
                period.split("(")[0] if "(" in period and period != "미상" else period
            )
    ...
    def _prepare_clustering_data_from_json(self, data):
        ...
        try:
            ...
            self.logger.warning(
                "BookMetadataExtractor를 로드할 수 없습니다. 메타데이터 없이 진행합니다."
            )
        ...
        if "books_summary" in data:
            for book in data["books_summary"]:
                pa_accuracy = (
                    book.get("pa_accuracy", 0) * 100
                )  # 0-1 범위를 0-100으로 변환
                sa_accuracy = (
                    book.get("sa_accuracy", 0) * 100
                )  # 0-1 범위를 0-100으로 변환
                ...
                if metadata_extractor and book_name:
                    try:
                        ...
                        # 상세 정보 가져오기 (시대, 분류)
                        detailed_info = metadata_extractor.get_detailed_author_info(
                            book_name
                        )
    ...
    def create_metadata_analysis(self):
        ...
        for i, author in enumerate(top_authors):
            ...
            ax1.scatter(
                author_subset["pa_accuracy"],
                author_subset["sa_accuracy"],
                label=author,
                alpha=0.7,
                s=60,
            )
        ...
        # 2. 시대별 품질 분포
        dynasty_quality = (
            self.clustering_df.groupby("dynasty")["quality_grade"]
            .mean()
        ...
        bars = ax2.bar(
            range(len(dynasty_quality)), dynasty_quality.values, color=colors, alpha=0.8
        )
        ax2.set_xlabel("시대")
        ...
        # 3. 분류별 품질 분포 (4부분류)
        genre_quality = (
            self.clustering_df.groupby("genre")["quality_grade"]
            .mean()
            .sort_values(ascending=False)
        )
        ...
        bars = ax3.bar(
            range(len(genre_quality)), genre_quality.values, color=colors, alpha=0.8
        )
        ax3.set_xlabel("4부분류")
        ...
        pivot_table = self.clustering_df.pivot_table(
            values="quality_grade", index="dynasty", columns="genre", aggfunc="mean"
        )
    ...
    def create_author_clustering_analysis(self, results):
        ...
        # 1. 클러스터별 작가 분포
        cluster_author = (
            df_with_clusters.groupby(["cluster", "author"]).size().unstack(fill_value=0)
        )
        ...
        author_cluster = (
            top_author_data.groupby(["author", "cluster"]).size().unstack(fill_value=0)
        )
    ...
    def find_elbow_point(self, k_range, inertias):
        ...
        # 2차 차분 계산 (기울기 변화율)
        second_diff = [
            first_diff[i] - first_diff[i + 1] for i in range(len(first_diff) - 1)
        ]
    ...
    def perform_clustering_analysis(self):
        ...
        print(f"📊 클러스터 수 후보:")
        print(
            f"   실루엣 점수 최적: k={silhouette_optimal} (점수: {max(silhouette_scores):.3f})"
        )
        print(
            f"   Calinski-Harabasz 최적: k={calinski_optimal} (점수: {max(calinski_scores):.1f})"
        )
        print(
            f"   Davies-Bouldin 최적: k={davies_optimal} (점수: {min(davies_scores):.3f})"
        )
        ...
        if silhouette_optimal == elbow_k:
            print(
                f"   ✅ 실루엣 점수와 엘보우 방법이 일치: k={silhouette_optimal} 강력 권장"
            )
        elif abs(silhouette_optimal - elbow_k) <= 1:
            print(
                f"   ✅ 실루엣 점수(k={silhouette_optimal})와 엘보우 방법(k={elbow_k})이 유사: 둘 다 고려 가능"
            )
        else:
            print(f"   ⚠️  지표들이 다른 결과를 제시: 도메인 지식으로 판단 필요")
            print(f"      - 큰 그룹 구분: k={min(silhouette_optimal, elbow_k)}")
            print(
                f"      - 세부 분석: k={max(silhouette_optimal, calinski_optimal, davies_optimal)}"
            )
        ...
        print("\n🛠 DBSCAN 하이퍼파라미터 제안:")
        print(
            f"   • 권장 eps: {best_dbscan['eps']}, min_samples: {best_dbscan['min_samples']}"
        )
        print(
            f"   • 예상 클러스터 수: {best_dbscan['n_clusters']}, 예상 노이즈: {best_dbscan['n_noise']}"
        )
    ...
    def plot_clustering_results(self, results, X, X_scaled, scaler):
        ...
        fig.suptitle(
            "PA/SA 점수 기반 비지도 클러스터링 분석", fontsize=16, fontweight="bold"
        )

        # 1. 원본 데이터 분포
        ax1 = axes[0, 0]
        scatter = ax1.scatter(
            self.clustering_df["pa_accuracy"],
            self.clustering_df["sa_accuracy"],
            c=self.clustering_df["quality_grade"],
            cmap="viridis",
            s=60,
            alpha=0.7,
            edgecolors="black",
        ...
        ax1.set_title(
            ...
            else "원본 데이터 분포 (품질 등급별 색상)"
        )
        ax1.grid(True, alpha=0.3)
        plt.colorbar(
            scatter,
            ax=ax1,
            label="Quality Grade" if not font_installed else "품질 등급",
        )
        ...
        ax2.set_title(
            f'K-means 클러스터링 (k={results["kmeans"]["optimal_k"]})\n'
        ...
        for k, col in zip(unique_labels, colors):
            ...
            ax3.scatter(
                self.clustering_df.loc[class_member_mask, "pa_accuracy"],
    ...
    def plot_elbow_method(self, X_scaled):
        ...
        ax1.set_xlabel(
            "Cluster Number (k)" if not font_installed else "클러스터 수 (k)"
        )
        ax1.set_ylabel("Inertia (WSS)")
        ax1.set_title(
            "Elbow Method - Inertia" if not font_installed else "엘보우 방법 - Inertia"
        )
        ax1.grid(True, alpha=0.3)
        ...
        ax3.axvline(
            ...
            linestyle="--",
            label=(
                f"Optimal k={calinski_optimal}"
                if not font_installed
                else f"최적 k={calinski_optimal}"
            ),
        )
        ax3.legend()
        ...
        ax4.set_title(
            ...
            if not font_installed
            else "Davies-Bouldin 지수 (낮을수록 좋음)"
        )
        ax4.grid(True, alpha=0.3)
        ...
        ax4.axvline(
            ...
            linestyle="--",
            label=(
                f"Optimal k={davies_optimal}"
                if not font_installed
                else f"최적 k={davies_optimal}"
            ),
        )
        ax4.legend()
    ...
    def plot_multiple_k_comparison(self, results, X, X_scaled, scaler):
        ...
        fig.suptitle(
            (
                "K-means Clustering Comparison (Different k values)"
                if not font_installed
                else "K-means 클러스터링 비교 (다양한 k값)"
            ),
            fontsize=14,
            fontweight="bold",
        )
    ...
    def find_cluster_representatives(
        self, cluster_data, cluster_centers, cluster_id, scaler
    ):
    ...
    def analyze_clusters(self, results):
        ...
        for cluster_id in sorted(df_kmeans["cluster"].unique()):
            ...
            print(
                f"  PA 정확도: {cluster_data['pa_accuracy'].mean():.1f}% ± {cluster_data['pa_accuracy'].std():.1f}%"
            )
            print(
                f"  SA 정확도: {cluster_data['sa_accuracy'].mean():.1f}% ± {cluster_data['sa_accuracy'].std():.1f}%"
            )
            print(
                f"  품질 등급: {cluster_data['quality_grade'].mean():.1f}% ± {cluster_data['quality_grade'].std():.1f}%"
            )
            ...
            if "author" in cluster_data.columns:
                ...
                if len(top_authors) > 0:
                    print(
                        f"  📝 주요 작가: {', '.join([f'{author}({count}권)' for author, count in top_authors.items()])}"
                    )
                ...
                if "dynasty" in cluster_data.columns:
                    ...
                    if len(dynasty_dist) > 0:
                        print(
                            f"  🏛️ 시대 분포: {', '.join([f'{dynasty}({count})' for dynasty, count in dynasty_dist.head(3).items()])}"
                        )
                ...
                if "genre" in cluster_data.columns:
                    ...
                    if len(genre_dist) > 0:
                        print(
                            f"  📚 분류 분포: {', '.join([f'{genre}({count})' for genre, count in genre_dist.head(3).items()])}"
                        )
            ...
            if representatives is not None and len(representatives) > 0:
                ...
                for idx, (_, rep) in enumerate(representatives.iterrows(), 1):
                    ...
                    print(
                        f"        PA: {pa_acc:.1f}%, SA: {sa_acc:.1f}% (중심거리: {distance:.3f})"
                    )
            ...
            print(
                f"  🏆 품질 우수 서종: {', '.join(top_quality['filename'].str.replace('.xml', '').tolist())}"
            )
        ...
        if "dbscan_tuning" in results:
            ...
            print(
                f"   • 예상 클러스터 수={rec['n_clusters']}, 노이즈={rec['n_noise']} ({noise_ratio*100:.1f}%)"
            )
            if rec["silhouette"] >= 0:
            ...
            if noise_ratio > 0.3:
                print(
                    "   - 노이즈가 많은 편입니다. eps를 소폭 증가(+0.05~0.1)하거나 min_samples를 완화(감소)해보세요."
                )
            elif rec["n_clusters"] <= 1:
                print(
                    "   - 클러스터가 하나만 형성됩니다. eps를 확대하거나 min_samples를 줄여 분할을 유도해보세요."
                )
            else:
                print(
                    "   - 현재 파라미터로 적정 분할이 이루어졌습니다. 도메인 기준으로 k-means 결과와 병행 검토를 권장합니다."
                )
            ...
            try:
                ...
                top_candidates = sorted(
                    candidates, key=lambda r: r.get("score", 0), reverse=True
                )[:10]
    ...
    def create_statistical_summary(self):
        ...
        if self.df is not None:
            ...
            print(
                f"📊 PA 정확도: 평균 {self.df['PA정확도(%)'].mean():.1f}% (최소 {self.df['PA정확도(%)'].min():.1f}%, 최대 {self.df['PA정확도(%)'].max():.1f}%)"
            )
            print(
                f"📊 SA 정확도: 평균 {self.df['SA정확도(%)'].mean():.1f}% (최소 {self.df['SA정확도(%)'].min():.1f}%, 최대 {self.df['SA정확도(%)'].max():.1f}%)"
            )
    ...
    def run_all_analysis(self):
        ...
        self.logger.info(
            f"통합 데이터 시각화 및 클러스터링 분석 시작 - 시작시간: {start_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        self.logger.info(
            f"분석 완료 - 종료시간: {end_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
