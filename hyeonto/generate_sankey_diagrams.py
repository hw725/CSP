...
def compute_flow_matrix(
    df1: pd.DataFrame, df2: pd.DataFrame, id_col: str = "left_sentence_id"
) -> dict:
    ...
    if id_col not in df1.columns or id_col not in df2.columns:
        ...
        merged = df1[[id_col, "cluster_id"]].merge(
            df2[[id_col, "cluster_id"]], on=id_col, suffixes=("_1", "_2")
        )
...
def generate_sankey_html(
    ...
    for _, row in flow_df.iterrows():
        ...
        links.append(
            {"source": source_idx, "target": target_idx, "value": int(row["count"])}
        )
...
def main():
    ...
    paths = {
        "sentence_k3": reports_dir / "sentence_k3_normalized" / "sentence_clusters.csv",
        "phrase_k3": reports_dir / "phrase_k3_normalized" / "phrase_clusters.csv",
    }
    ...
    # Sentence K=3 ↔ Phrase K=3 (서종 기준)

    if "sentence_k3" in data and "phrase_k3" in data:

        print("? Sentence K=3 ↔ Phrase K=3 (서종 기준)")

        # 서종별 클러스터 분포 계산

        sentence_book_cluster = (
            data["sentence_k3"]
            .groupby(["book", "cluster_id"])
            .size()
        ...
        sentence_totals = (
            ...
            .groupby("book")
            .size()
            .reset_index(name="sentence_total")
        )

        phrase_totals = (
            data["phrase_k3"].groupby("book").size().reset_index(name="phrase_total")
        )

        sentence_book_cluster = sentence_book_cluster.merge(sentence_totals, on="book")

        phrase_book_cluster = phrase_book_cluster.merge(phrase_totals, on="book")

        sentence_book_cluster["sentence_ratio"] = (
            sentence_book_cluster["sentence_count"]
            / sentence_book_cluster["sentence_total"]
        )

        phrase_book_cluster["phrase_ratio"] = (
            phrase_book_cluster["phrase_count"] / phrase_book_cluster["phrase_total"]
        )
        ...
        common_books = set(sentence_book_cluster["book"].unique()) & set(
            phrase_book_cluster["book"].unique()
        )

        for book in common_books:

            sentence_book = sentence_book_cluster[sentence_book_cluster["book"] == book]

            phrase_book = phrase_book_cluster[phrase_book_cluster["book"] == book]

            book_weight = min(
                sentence_book["sentence_total"].iloc[0],
                phrase_book["phrase_total"].iloc[0],
            )
            ...
            for _, sentence_row in sentence_book.iterrows():
                ...
                for _, phrase_row in phrase_book.iterrows():
                    ...
                    weight = (
                        sentence_row["sentence_ratio"]
                        * phrase_row["phrase_ratio"]
                        * book_weight
                    )
        ...
        flow = (
            pd.DataFrame(flow_list)
            .groupby(["cluster1", "cluster2"])["count"]
            .sum()
            .reset_index()
        )
        ...
        generate_sankey_html(
            ...
            3,
            3,
            output_dir / f"sankey_sentence3_phrase3{suffix}.html",
            "Sentence K=3 ↔ Phrase K=3 (서종 분포 기반)",
