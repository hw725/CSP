...
MARKER_PATTERN = re.compile(
    r"[가-힣]+(?:라|니|하니|하여|하고|하야|하다|는|은|를|을|에|로|면|요|니라|이라|잇고|잇가|시니|러니|되|대)"
)
...
def compute_genre_specificity(
    ngram_results: Dict, genre: str, n: int
...
def write_ngram_report(
    out_dir: Path, ngram_results: Dict, df: pd.DataFrame, analysis_type: str
) -> None:
    """n-gram 분석 리포트 작성"""
    ...
    for n in sorted(ngram_results.keys()):
        ...
        if canon_ngrams:
            ...
            for ng, cnt in canon_ngrams.items():
                if data["all"][ng] > 0:
                    expected = (
                        (data["all"][ng] / total_all) * total_canon
                        if total_all > 0
                        else 0
                    )
            ...
            for i, (ng, canon_cnt, all_cnt, ratio) in enumerate(specificity[:20], 1):
                lines.append(
                    f"| {i} | `{ng}` | {canon_cnt:,} | {all_cnt:,} | {ratio:.2f}x |"
                )
            lines.append("")
        ...
        for cluster in sorted(data["by_cluster"].keys()):
            ...
            top3 = [
                f"`{ng}`({cnt})" for ng, cnt in Counter(cluster_data).most_common(3)
            ]
    ...
    if 2 in ngram_results:
        ...
        for ng, ratio, cnt in compute_genre_specificity(ngram_results, "논어집주", 2)[
            :5
        ]:
...
def main() -> None:
    ...
    p.add_argument(
        "--input", type=str, required=True, help="입력 CSV (boundary_clusters.csv)"
    )
    ...
    p.add_argument(
        "--n-values", type=str, default="2,3", help="분석할 n 값들 (콤마 구분)"
    )
