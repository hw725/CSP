...
def parse_xml_file(xml_path):
    ...
    for s_elem in root.iter("s"):
        ...
        sentences.append({"sentence_id": s_id, "text": text})
    return sentences
...
def main():
    ...
    print("\n=== Sentence 데이터 병합 ===")
    # 컬럼 통일
    sentence_43_renamed = sentence_43.rename(
        columns={
            "문단식별자": "문단식별자",
            "문장식별자": "문장식별자",
            "원문": "원문",
    ...
    print("\n=== 검증: 잇고/잇가 보존 ===")
    print(
        f"  Sentence 잇고: {sentence_full['원문'].astype(str).str.contains('잇고').sum()}"
    )
    print(
        f"  Sentence 잇가: {sentence_full['원문'].astype(str).str.contains('잇가').sum()}"
    )
