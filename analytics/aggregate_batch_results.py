...
def collect_all_evaluations(results_dir: Path) -> dict:
    ...
    return {"pa_evaluations": all_pa_evals, "sa_evaluations": all_sa_evals}
...
def generate_report(results_dir: Path, output_dir: Path) -> None:
    ...
    pa_metrics = [
        "avg_exact_match",
        "avg_segment_count_match",
        "avg_text_match",
        "avg_source_text_match",
        "avg_target_text_match",
        "avg_partial_match",
    ...
    for metric, stat in pa_stats.items():
        ...
        print(
            f"  사분위: Q25={stat['q25']:.4f}, Q75={stat['q75']:.4f}, Q90={stat['q90']:.4f}"
        )
    ...
    report = {
        ...
        "summary": {"total_books_pa": len(pa_df), "total_books_sa": len(sa_df)},
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output-dir", default="analytics", help="통계 출력 디렉터리 (기본: analytics)"
    )
