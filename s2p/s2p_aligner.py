...
class OpenAIWrapper:
    ...
    def __init__(self, max_workers=4, batch_size=100):
        try:
            ...
            logger.debug(
                f"✅ OpenAI 임베더 초기화 완료 (max_workers={max_workers}, batch_size={batch_size})"
            )
                            ...
                            try:
                                ...
                                if inspect.isclass(obj) or inspect.isfunction(obj):
                                    ...
                                    if (
                                        ...
                                        or name.startswith("_create")
                                        or name.startswith("_import")
                                    ):
                                        to_allow.append(obj)
                            except Exception:
                                continue
...
def _adjust_segments_to_count(
    ...
    if (
        text
        and src_units
        and len(src_units) == target_count
        and refine_boundaries_with_llm
    ):
        try:
            adjusted = _adjust_with_llm_semantic(
                text, segments, src_units, target_count, mode
            )
            if adjusted and len(adjusted) == target_count:
                ...
                if original_flat == adjusted_flat:
                    logger.info(
                        f"✅ LLM 의미 기반 조정 성공: {len(segments)}개 → {target_count}개"
                    )
                    return adjusted
        except Exception as e:
...
{{"segments": ["세그먼트1", "세그먼트2", ...]}}"""
    ...
    try:
        ...
        if not isinstance(adjusted, list) or len(adjusted) != target_count:
            logger.warning(
                f"LLM 의미 조정: 잘못된 개수 반환 ({len(adjusted)} != {target_count})"
            )
            return None
    ...
    **kwargs,
    ...
    if precomputed_segments is not None:
        if len(precomputed_segments) == src_units_count:
            logger.debug(
                f"🚀 사전 계산된 경계 결과 사용: {len(precomputed_segments)}개 세그먼트"
            )
            return precomputed_segments
        else:
            # 개수 불일치 시에도 사전 계산 결과를 기반으로 조정
            logger.debug(
                f"⚠️ 사전 계산 결과 개수 불일치: {len(precomputed_segments)}개 vs 원문 {src_units_count}개"
            )
    ...
    if use_semantic and src_units and len(src_units) == src_units_count:
        try:
            ...
            if result_normalized != original_normalized:
                ...
                logger.warning(
                    f"⚠️ 의미 기반 분할 무결성 경고: 원본 {len(original_normalized)}자 != 분할 {len(result_normalized)}자 (차이: {diff:+d}자)"
                )
    ...
    if "result" not in locals():
        result = _split_tgt_by_src_units_simple(
            text, src_units_count, src_units=src_units, **kwargs
        )
        ...
        if result_normalized != original_normalized:
            diff = len(original_normalized) - len(result_normalized)
            logger.warning(
                f"⚠️ 단순 분할 무결성 경고: 원본 {len(original_normalized)}자 != 분할 {len(result_normalized)}자 (차이: {diff:+d}자)"
            )
            ...
            if abs(diff) > 10:
                ...
                if result_normalized_2 == original_normalized:
                    ...
                    logger.error(
                        f"❌ 균등 분할도 무결성 문제: 차이 {len(original_normalized) - len(result_normalized_2):+d}자"
                    )
    ...
    if refine_boundaries_with_llm:
        try:
            ...
            try:
                if (
                    src_units
                    and isinstance(src_units, list)
                    and len(src_units) == src_units_count
                ):
            ...
            if llm_checked:
                ...
                if not same_text:
                    ...
                    logger.info(
                        f"🔧 LLM 경계 조정: {len(llm_checked)}개 → {src_units_count}개 (추가 분할)"
                    )
                    ...
                    logger.info(
                        f"🔧 LLM 경계 조정: {len(llm_checked)}개 → {src_units_count}개 (병합)"
                    )
                    adjusted = _adjust_segments_to_count(
                        llm_checked,
                        src_units_count,
...
def _split_tgt_by_src_units_semantic(
    src_units: List[str], tgt_text: str, min_tokens: int = 1, **kwargs
) -> List[str]:
    ...
    try:
        ...
        embedder_device = kwargs.get(
            "embedder_device", kwargs.get("device", os.getenv("CSP_DEVICE", "cuda"))
        )
        embedder_device_id = kwargs.get("embedder_device_id", None)
        if (
            embedder_device_id is None
            and embedder_device
            and embedder_device.lower() == "cuda"
        ):
        ...
        if embedder_name.lower() == "openai":
            ...
            logger.debug(
                f"✅ OpenAI 임베더로 순서 보장 의미 매칭 시작 (max_workers={max_workers})"
            )
            ...
            compute_embeddings_func = get_embedder(
                embedder_name, device_id=embedder_device_id
            )
            logger.debug(
                f"✅ {embedder_name.upper()} 임베더로 순서 보장 의미 매칭 시작 (device_id={embedder_device_id})"
            )
        ...
        try:
            ...
            src_embeddings = compute_embeddings_func(src_units, batch_size=batch_size)
        ...
        try:
            ...
            tgt_embeddings = compute_embeddings_func(
                tgt_tokens_aug, batch_size=batch_size
            )
        ...
        if optimal_split and len(optimal_split) == N:
            logger.debug(
                f"✅ {embedder_name.upper()} 순서 보장 분할 성공: {len(optimal_split)}개 단위"
            )
            ...
            if optimal_split:
                logger.error(
                    f"❌ DP 분할 실패: 기대 {N}개, 실제 {len(optimal_split)}개 → 폴백"
                )
            else:
        ...
        for i in range(N):
            ...
            if start_idx < T:
                ...
                if start_idx >= len(tgt_tokens_original):
                    logger.error(
                        f"🚨 인덱스 오류: start_idx={start_idx}, len(tgt_tokens_original)={len(tgt_tokens_original)}"
                    )
                    unit_text = ""
                elif end_idx > len(tgt_tokens_original):
                    # 경계를 초과한 경우 (이론적으로 발생 불가, 하지만 안전성 위해)
                    logger.warning(
                        f"⚠️ 경계 초과: end_idx={end_idx}, len(tgt_tokens_original)={len(tgt_tokens_original)}"
                    )
...
def _find_optimal_split_dp_sequential(
    ...
    **kwargs,
    ...
    distance_decay_alpha: float = float(
        kwargs.get("distance_decay", cfg_params.get("distance_decay", 0.05))
    )
    boundary_bonus: float = float(
        kwargs.get("boundary_bonus", cfg_params.get("boundary_bonus", 0.15))
    )
    particle_bonus: float = float(
        kwargs.get("particle_bonus", cfg_params.get("particle_bonus", 0.2))
    )
    length_penalty_alpha: float = float(
        kwargs.get("length_penalty", cfg_params.get("length_penalty", 0.1))
    )
    sim_gamma: float = float(kwargs.get("sim_gamma", cfg_params.get("sim_gamma", 1.2)))
    similarity_threshold: float = float(
        kwargs.get("similarity_threshold", cfg_params.get("similarity_threshold", 0.5))
    )
    ...
    boundary_flags = (
        boundary_flags_orig
        if len(boundary_flags_orig) == len(boundary_flags)
        else boundary_flags
    )
    particle_flags = (
        particle_flags_orig
        if len(particle_flags_orig) == len(particle_flags)
        else particle_flags
    )
    ...
    try:
        ...
        tokens_for_analysis = (
            tgt_tokens_orig if len(tgt_tokens_orig) == len(tgt_tokens) else tgt_tokens
        )
    ...
    for t in range(T):
        for s in range(N):
            sim = np.dot(tgt_embeddings[t], src_embeddings[s]) / (
                np.linalg.norm(tgt_embeddings[t]) * np.linalg.norm(src_embeddings[s])
                + 1e-8
            )
            sim_val = float(sim)
            if sim_gamma != 1.0:
    ...
    allow_parse = (syntax_when == "always") or (
        syntax_when == "ambiguous" and is_ambiguous(enhanced_similarity)
    )
                ...
                for idx, (s_off, e_off) in enumerate(token_spans):
                    ...
                    if (e_off - 1) in strength_map:
                        ko_boundary_token_strengths[idx] = max(
                            ko_boundary_token_strengths[idx],
                            float(strength_map[e_off - 1]),
                        )
        except Exception:
            pass
    ...
    if allow_parse and syntax_hints in ("zh", "both"):
        try:
            ...
            src_boundary_indices = get_chinese_unit_boundary_indices(
                kwargs.get("src_units", [])
            )
        except Exception:
            src_boundary_indices = set()
    ...
    if tgt_tokens_orig is None:
        logger.error(
            "🚨 CRITICAL: tgt_tokens_original이 DP에 전달되지 않았습니다! 원본 텍스트가 손상될 수 있습니다."
        )
    ...
    while j > 0:
        ...
        if start >= 0:
            ...
            if start < 0 or end > len(tgt_tokens_orig) or start > end:
                logger.error(
                    f"🚨 인덱스 오류: start={start}, end={end}, len(original)={len(tgt_tokens_orig)}"
                )
                return None
...
def _split_tgt_by_src_units_simple(
    text: str, target_count: int, src_units: List[str] = None, **kwargs
) -> List[str]:
    ...
    try:
        ...
        cand = (
            split_target_sentences_advanced(
                text, max_length=400, splitter="punctuation"
            )
            or []
        )
        offset = 0
...
def process_single_row(row_data: Dict[str, Any], **kwargs) -> List[Dict[str, Any]]:
                    ...
                    try:
                        ...
                        if "src_text" in sig.parameters or len(sig.parameters) >= 2:
                            tgt_units_by_model = boundary_model.segment_text(
                                ...
                                n_segments=(
                                    len(src_units) if len(src_units) > 1 else None
                                ),
                                threshold=threshold,
                            )
                        else:
                        ...
                        logger.debug(
                            f"DEBUG: 모델 예측 세그먼트 (On-the-fly): {len(tgt_units_by_model)}"
                        )
                    except TypeError:
                        tgt_units_by_model = boundary_model.segment_text(
                            str(translation_text),
                        ...
                        logger.debug(
                            f"DEBUG: 폴백 모델 예측 세그먼트 (On-the-fly): {len(tgt_units_by_model)}"
                        )
                else:
                    logger.debug(
                        f"🚀 사전 계산된 경계 결과 사용 (Batch): {len(tgt_units_by_model)}개"
                    )
                ...
                if tgt_units_by_model and 2 <= len(tgt_units_by_model) <= max_segments:
                    ...
                    if tgt_ok:
                        ...
                        if alignment_model is not None:
                            src_units_by_model = alignment_model.match_segments(
                                src_units, tgt_units_by_model
                            )
                        else:
                            # 🆕 Alignment 모델 없이: DP로 원문을 번역문 개수에 맞춰 재분할
                            logger.debug(
                                f"DEBUG: DP 재분할 시도 (Target Count: {len(tgt_units_by_model)})"
                            )
                            src_units_by_model = split_tgt_meaning_units(
                                source_text,
                                len(tgt_units_by_model),
                                src_units=[source_text],
                                use_semantic=False,
                                **kwargs,
                            )
                            logger.debug(
                                f"DEBUG: DP 재분할 결과 수: {len(src_units_by_model)}"
                            )

                        # 원문 무결성 검증
                        src_ok = "".join(str(source_text).split()) == "".join(
                            "".join(src_units_by_model).split()
                        )
    ...
    try:
        ...
        if embedder_name.lower() != "none":
            if embedder_name.lower() == "openai":
                ...
                compute_embeddings = get_embedder(
                    embedder_name, device_id=embedder_device_id
                )

            all_texts = src_units + trans_units
            embeddings = compute_embeddings(
                all_texts, batch_size=kwargs.get("batch_size", 100)
            )
    ...
    for _, (src_unit, trans_unit, sim) in enumerate(
        zip(src_units, trans_units, segment_similarities), start=1
    ):
    ...
    try:
        ...
        if input_trans_for_check != processed_trans:
            ...
            logger.error(
                f"  길이 차이: {len(processed_trans) - len(input_trans_for_check)}자"
            )

            # 문자 정확도 계산
            correct_chars = sum(
                1 for a, b in zip(input_trans_for_check, processed_trans) if a == b
            )
            accuracy = correct_chars / max(
                len(input_trans_for_check), len(processed_trans)
            )
            ...
            if len(input_trans_for_check) > 0 and len(processed_trans) > 0:
                ...
                if diff:
                    ...
                    logger.error(
                        f"  처리 샘플: '{processed_trans[:50]}{'...' if len(processed_trans) > 50 else ''}'"
                    )
...
def align_translation_to_source(
    src_units: List[str], translation: str, use_semantic: bool = True, **kwargs
) -> List[str]:
    """
        ...
        if embedder_name and embedder_name.lower() != "none":
            ...
            for src_unit, tgt_unit in zip(src_units, trans_units):
                if src_unit.strip() and tgt_unit.strip():
                    try:
                        src_emb = compute_embeddings_func(
                            [src_unit], batch_size=batch_size
                        )[0]
                        tgt_emb = compute_embeddings_func(
                            [tgt_unit], batch_size=batch_size
                        )[0]
                        sim = float(
                            np.dot(src_emb, tgt_emb)
