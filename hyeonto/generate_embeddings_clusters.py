...
from hyeonto.scripts.hyeonto_normalizer import normalize_hyeonto_marker
...
def prepare_data_for_clustering(input_path: Path, output_dir: Path, boundary_type: str):
    ...
    df["marker_normalized"] = df["marker"].apply(lambda x: normalize_markers(x.split(",")) if pd.notna(x) and x else "")
...
def main():
    ...
    parser.add_argument("--boundary", choices=["sentence", "phrase", "both"], default="both", help="분석할 경계 유형")
    parser.add_argument("--sentence-k", type=int, nargs="+", default=[4, 14], help="Sentence 클러스터 K 값들")
    parser.add_argument("--phrase-k", type=int, nargs="+", default=[4, 24], help="Phrase 클러스터 K 값들")
