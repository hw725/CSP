...
def compute_embeddings_with_cache(
    ...
    logger.info(
        f"✅ OpenAI 임베딩 완료: {len(all_embeddings)}개 (병렬 워커: {max_workers})"
    )
...
def get_embedding_dimension(
    model: str = "text-embedding-3-large",
) -> int:  # 또는 "text-embedding-3-small", "text-embedding-ada-002"
    """임베딩 차원 반환"""
...
def test_openai_connection(
    model: str = "text-embedding-3-large",
) -> bool:  # 또는 "text-embedding-3-small", "text-embedding-ada-002"
    """OpenAI 연결 테스트"""
    ...
    try:
        ...
        test_embeddings = compute_embeddings_with_cache(
            ["테스트 문장입니다."], model=model, use_cache=False
        )
