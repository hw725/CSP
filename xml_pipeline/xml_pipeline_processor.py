"""
XML 파이프라인 프로세서 (수정됨) - 올바른 XML 구조 매핑
...
class XMLProcessor:
            ...
            for orig_elem in orig_elements:
                ...
                if matching_trans is not None:
                    ...
                    if orig_text.strip() and trans_text.strip():
                        data.append({
                            '문단식별자': orig_id,  # XML ID를 문단식별자로 사용
                            '원문': orig_text.strip(),
                            '번역문': trans_text.strip()
                        })
                        print(f"🔗 문단 {orig_id}: 원문 {len(orig_text.strip())}자, 번역문 {len(trans_text.strip())}자")
            
            print(f"✅ 추출된 문단 쌍 수: {len(data)}")
            return pd.DataFrame(data)
            
        except Exception as e:
            print(f"❌ XML 문단 추출 오류: {e}")
            raise Exception(f"XML 문단 추출 실패: {e}")
    
    @staticmethod
    def extract_sentence_data(orig_xml: str, trans_xml: str) -> pd.DataFrame:
        """XML 쌍에서 문장병렬 데이터 추출 (SA 입력용)"""
        try:
            print(f"🔍 문장 데이터 추출 시작:")
            print(f"   원문: {Path(orig_xml).name}")
            print(f"   번역: {Path(trans_xml).name}")
            ...
            for orig_elem in orig_elements:
                ...
                # 매칭되는 번역문 찾기 - 번역문에서는 "_T" 패턴으로 매칭
                matching_trans = None
                for trans_elem in trans_elements:
                    trans_id = trans_elem.get('식별자')
                    if trans_id and orig_id and trans_id == orig_id + "_T":
                        matching_trans = trans_elem
                        break
                
                if matching_trans is not None:
                    # s 요소들 추출 (문장 역할)
                    orig_s_elements = orig_elem.findall('.//s')
                    trans_s_elements = matching_trans.findall('.//s')
                    
                    # s 요소별 매칭 (문장 단위)
                    for orig_s, trans_s in zip(orig_s_elements, trans_s_elements):
                        # s 요소 내 모든 w 요소 연결
                        orig_text = XMLProcessor._join_w_texts(orig_s)
                        trans_text = XMLProcessor._join_w_texts(trans_s)
    ...
    def extract_phrase_data(orig_xml: str, trans_xml: str) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"✅ 추출된 구 쌍 수: {len(data)}")
            return pd.DataFrame(data)
            
        except Exception as e:
            print(f"❌ XML 구 추출 오류: {e}")
            raise Exception(f"XML 구 추출 실패: {e}")
    
    @staticmethod
    def extract_sentence_data(orig_xml: str, trans_xml: str) -> pd.DataFrame:
        """XML 쌍에서 문장병렬 데이터 추출 (PA 비교용)"""
        try:
            print(f"🔍 문장 데이터 추출 시작:")
            print(f"   원문: {Path(orig_xml).name}")
            print(f"   번역: {Path(trans_xml).name}")
