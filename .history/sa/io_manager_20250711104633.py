...
def process_batch_sentences(sentence_batch, device_id=None):
    """배치 단위 문장 처리 - 10문장 단위에 최적화된 모니터링"""
    global worker_embed_func, worker_modules
    ...
    try:
        if verbose_mode:
            print(f"🔄 워커 {mp.current_process().pid}: {batch_size}개 문장 배치 시작 (10문장 단위)")
        
        for idx, sentence_data in enumerate(sentence_batch):
            try:
                # 10문장 단위이므로 2-3문장마다 진행 상황 출력
                if batch_size <= 10:
                    # 10문장 이하: 3개마다 또는 마지막
                    if (idx + 1) % 3 == 0 or (idx + 1) == batch_size:
        ...
        # 처리 시간이 예상보다 길면 경고
        expected_time = batch_size * 10  # 문장당 10초 예상
...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
    ...
    try:
        if parallel and total_sentences > workers:
            ...
            # 배치 크기를 10문장으로 고정 (핵심 변경사항)
            batch_size_per_worker = 10  # 모든 경우에 10문장으로 고정
            
            # 예외적으로 매우 작은 데이터셋의 경우만 조정
            if total_sentences <= 20:
                batch_size_per_worker = 5   # 20개 이하: 배치당 5개
            elif total_sentences <= 50:
                batch_size_per_worker = 8   # 50개 이하: 배치당 8개
            else:
                batch_size_per_worker = 10  # 기본: 배치당 10개
            ...
            print(f"   예상 배치당 처리 시간: {batch_size_per_worker * 10}초 (문장당 10초)")
            ...
            with mp.Pool(processes=workers, initializer=init_worker, 
                ...
                for batch_idx, async_result, batch_size in async_results:
                    try:
                        # 타임아웃을 문장당 10초로 유지
                        timeout_seconds = max(600, batch_size * 20)  # 문장당 10초, 최소 5분
                        
                        # 10문장 단위이므로 모든 배치 상태 출력
                        print(f"⏳ 배치 {batch_idx+1}/{len(sentence_batches)} 처리 중... (크기: {batch_size}, 예상시간: {timeout_seconds}초)")
