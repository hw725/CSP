...
def extract_markers_from_v6_row(
    row: pd.Series, cols: list[str]
) -> list[tuple[str, bool]]:
    """V6 행에서 (현토, 문장끝여부) 추출"""
    results = []
    for col in cols:
...
def main():
    ...
    rdf.to_csv(
        args.out_dir / "marker_syntactic_profile.csv", index=False, encoding="utf-8-sig"
    )
    ...
    try:
        ...
        fig.write_html(
            str(args.out_dir / "syntactic_profile_scatter.html"), include_plotlyjs="cdn"
        )
    except:
        pass
