...
try:
    from s2p.punctuation import (
        integrity_guard,
        safe_mask_brackets,
        safe_restore_brackets,
...
class SafeFileProcessor:
    ...
    def __init__(
        self, max_workers: int = 4, chunk_size: int = 100, verbose: bool = False
    ):
    ...
    def process_file_with_integrity(
        self, input_file: str, output_file: str, processing_function, **kwargs
    ) -> bool:
        """무결성 보장 파일 처리"""
        ...
        try:
            ...
            if empty_rows > 0:
                logger.info(
                    f"⚠️ 빈 원문/번역문 {empty_rows}개 포함 (누락 방지를 위해 유지)"
                )
            ...
            for i, chunk in enumerate(chunks):
                ...
                print(
                    f"\n🔧 청크 {i+1}/{len(chunks)} 시작 ({len(chunk)}행)...",
                    flush=True,
                )
                ...
                if embedder_name.lower() != "none":
                    try:
                        ...
                        print(
                            f"   ✅ 청크 임베딩 계산: {len(chunk_texts_list)}개",
                            flush=True,
                        )
                ...
                if use_boundary_model or use_phrase_alignment:
                    try:
                        ...
                        for batch_start in range(0, len(chunk_src), batch_size):
                            ...
                            for idx, segments in zip(
                                chunk_row_indices[batch_start:batch_end], segments_batch
                            ):
                                batch_boundary_cache[idx] = segments

                        print(
                            f"   ✅ 청크 경계 계산: {len(batch_boundary_cache)}개",
                            flush=True,
                        )
                    except Exception as e:
                        if self.verbose:
                ...
                if chunk_results:
                    ...
                    checkpoint_path = Path(
                        str(output_file).replace(".xlsx", "_checkpoint.csv")
                    )
                    try:
                        ...
                        if i == 0:
                            ...
                            # 이후 청크: 헤더 없이 append
                            chunk_df.to_csv(
                                checkpoint_path, index=False, mode="a", header=False
                            )
                        print(
                            f"💾 체크포인트 저장: 청크 {i+1}/{len(chunks)} ({len(results):,}개 누적)",
                            flush=True,
                        )
                    except Exception as e:
            ...
            if results:
                ...
                if final_integrity["valid"]:
                    ...
                    try:
                            ...
                            losses.append(
                                {
                                    ...
                                    "difference": diff,
                                    "loss_rate": (
                                        abs(diff) / len(original_src) * 100
                                        if len(original_src) > 0
                                        else 0
                                    ),
                                }
                            )
                            ...
                            losses.append(
                                {
                                    ...
                                    "difference": diff,
                                    "loss_rate": (
                                        abs(diff) / len(original_tgt) * 100
                                        if len(original_tgt) > 0
                                        else 0
                                    ),
                                }
                            )
                        ...
                        if losses:
                            ...
                            if not str(output_file).endswith(".csv"):
                                with pd.ExcelWriter(
                                    output_file, engine="openpyxl", mode="a"
                                ) as writer:
                                    integrity_losses_df.to_excel(
                                        writer,
                                        index=False,
                                        sheet_name="integrity_losses",
                                    )

                            for loss in losses:
                                print(
                                    f"⚠️ {loss['type']} 무결성 손실: {loss['difference']:,}자 ({loss['loss_rate']:.2f}%)"
                                )
                        else:
                    ...
                    if self.verbose:
                        ...
                        logger.info(
                            f"처리 통계: 성공 {self.processed_count}, 실패 {self.error_count}, 무결성실패 {self.integrity_failures}"
                        )
                    ...
                    if self.verbose:
                        logger.error(
                            f"최종 무결성 검증 실패: {final_integrity['message']}"
                        )
                    # 복구된 결과로 저장
                    if final_integrity.get("restored_df") is not None:
                        if str(output_file).endswith(".csv"):
                            final_integrity["restored_df"].to_csv(
                                output_file, index=False
                            )
                        else:
                            final_integrity["restored_df"].to_excel(
                                output_file, index=False
                            )
                        if self.verbose:
    ...
    def _register_file_integrity(self, df: pd.DataFrame, file_id: str):
        ...
        if "원문" in df.columns and "번역문" in df.columns:
            ...
            try:
                integrity_guard.register_original(
                    f"{file_id}_src_all", total_src, "file_input_src"
                )
                integrity_guard.register_original(
                    f"{file_id}_tgt_all", total_tgt, "file_input_tgt"
                )
                logger.info(
                    f"파일 무결성 등록: 원문 {len(total_src)}자, 번역문 {len(total_tgt)}자"
                )
            except Exception as e:
    ...
    def _process_chunk_with_integrity(
        ...
        **kwargs,
        ...
        for i, (idx, row) in enumerate(chunk.iterrows()):
            try:
                ...
                if i % 50 == 0 or i == chunk_size - 1:
                    ...
                    print(
                        f"  📍 청크 진행: {i+1}/{chunk_size} ({(i+1)*100//chunk_size}%) | {rate:.1f}행/초 | ETA: {eta:.0f}초",
                        flush=True,
                    )
                ...
                if src_text.strip() and tgt_text.strip():
                    if self.integrity_enabled:
                        try:
                            integrity_guard.register_original(
                                f"{row_id}_src", src_text, "row_input_src"
                            )
                            integrity_guard.register_original(
                                f"{row_id}_tgt", tgt_text, "row_input_tgt"
                            )
                        except Exception as e:
                    ...
                    if row_result:
                        # 행별 무결성 검증 (안전)
                        if self.integrity_enabled and self._verify_row_integrity(
                            row, row_result, row_id
                        ):
                            results.extend(
                                row_result
                                if isinstance(row_result, list)
                            ...
                            self.processed_count += (
                                len(row_result) if isinstance(row_result, list) else 1
                            )
        ...
        elapsed = time.time() - chunk_start_time
        print(
            f"\n  ✅ 청크 완료: {chunk_size}행 처리 ({elapsed:.1f}초, {chunk_size/elapsed:.1f}행/초)",
            flush=True,
        )
    ...
    def _verify_row_integrity(
        self, original_row: pd.Series, processed_results: List[Dict], row_id: str
    ) -> bool:
        """행별 무결성 검증 (안전 버전)"""
        ...
        try:
            ...
            # 처리된 결과에서 원문과 번역문 결합
            processed_src = "".join(
                [str(result.get("원문", "")) for result in processed_results]
            )
            processed_tgt = "".join(
                [str(result.get("번역문", "")) for result in processed_results]
            )

            # 무결성 검증
            src_valid, src_msg, _ = integrity_guard.verify_integrity(
                f"{row_id}_src", processed_src, "row_output_src"
            )
            tgt_valid, tgt_msg, _ = integrity_guard.verify_integrity(
                f"{row_id}_tgt", processed_tgt, "row_output_tgt"
            )
    ...
    def _verify_final_integrity(
        self, original_df: pd.DataFrame, result_df: pd.DataFrame, file_id: str
    ) -> Dict:
        """최종 무결성 검증 (안전 버전)"""
        ...
        try:
            ...
            if not src_valid:
                ...
                if success and len(restored_src_parts) == len(result_df):
                    ...
                    # 원본을 결과 개수만큼 분할
                    src_parts = self._split_text_by_count(
                        original_src_all, len(result_df)
                    )
            ...
            if not tgt_valid:
                ...
                if success and len(restored_tgt_parts) == len(result_df):
                    ...
                    # 원본을 결과 개수만큼 분할
                    tgt_parts = self._split_text_by_count(
                        original_tgt_all, len(result_df)
                    )
    ...
    **kwargs,
    ...
    if verbose:
        ...
        if use_boundary_model:
            logger.info(
                f"🔧 경계 모델 사용: threshold={boundary_threshold}, device={device}"
            )
...
def process_file_fallback(input_file: str, output_file: str, **kwargs) -> bool:
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            sent_id = (
                row.get("문장식별자", idx + 1)
                if "문장식별자" in row and pd.notna(row.get("문장식별자"))
                else idx + 1
            )
...
# 무결성 보장 처리 함수들
def safe_process_single_text(
    text: str, processing_function, text_id: str = None, **kwargs
) -> Any:
...
def safe_process_text_list(
    text_list: List[str], processing_function, list_id: str = None, **kwargs
) -> List[str]:
    ...
    try:
        ...
        for i, text in enumerate(text_list):
            if text.strip():
                ...
                result = safe_process_single_text(
                    text, processing_function, item_id, **kwargs
                )
                results.append(result)
            else:
                results.append(text)
        ...
        is_valid, message, info = integrity_guard.verify_integrity(
            list_id, combined_result, "list_output"
        )
...
def process_chunks_parallel(
    chunks: List[Any], processing_function, max_workers: int = 4, **kwargs
) -> List[Any]:
    """병렬 청크 처리"""
    ...
    if max_workers == 1:
        ...
        for i, chunk in enumerate(chunks):
            ...
            if chunk_result:
                results.extend(
                    chunk_result if isinstance(chunk_result, list) else [chunk_result]
                )
    else:
        # 병렬 처리
    ...
    try:
        ...
        if (use_boundary_model or use_phrase_alignment) and not hasattr(safe_process_s2p_row, "_boundary_model"):
            ...
            alignment_candidates = [
                Path("models/dual_encoder_alignment_s2p.pt"),
                Path("models/dual_encoder_alignment_sa.pt"),
            ]
            alignment_model_path = next(
