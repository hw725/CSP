...
def _call_ollama(
    prompt: str, model: str, system_prompt: str, debug: bool
) -> Optional[str]:
    """Call Ollama API."""
...
def _call_llm(
    prompt: str, model: str, backend: str, system_prompt: str, debug: bool
) -> Optional[str]:
...
def refine_boundaries_with_llm(
    ...
    _dbg(
        debug,
        f"invoked: segs={len(segments)} max={max_segments} task={task} ref={bool(reference_text)}",
    )
    ...
    model_name = model or os.getenv(
        "LLM_BOUNDARY_MODEL", default_models.get(backend, "qwen3-coder:480b-cloud")
    )
    ...
    try:
        ...
        if not (
            isinstance(new_segments, list)
            and all(isinstance(s, str) for s in new_segments)
        ):
...
def _build_prompt(
    text: str, segments: List[str], task: str, reference_text: str | None
) -> str:
    bullet = "\n - "
    prompt = (
