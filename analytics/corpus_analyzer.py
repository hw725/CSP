...
class CumulativeBookAnalyzer:
    ...
    def __init__(self, base_dir: str = None, preserve_manual_csv: bool = True):
        ...
        if (
            preserve_manual_csv
            and Path("cumulative_analysis_results_manual.csv").exists()
        ):
            self.csv_export_path = f"cumulative_analysis_results_auto_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
            print(
                f"⚠️  수동 편집된 CSV 파일을 보호하기 위해 새로운 파일로 저장합니다: {self.csv_export_path}"
            )
        else:
        ...
        # XLSX 로더 유틸 사용 준비
        try:
            from utils.xlsx_loader import list_books, load_p2s, load_s2p

            self._xlsx_list_books = list_books
            self._xlsx_load_p2s = load_p2s
            self._xlsx_load_s2p = load_s2p
            self.xlsx_mode = True
            logger.info("XLSX 모드 활성화: xlsx_pipeline_results를 사용합니다")
        except Exception:
            self.xlsx_mode = False
            logger.warning(
                "XLSX 로더를 사용할 수 없습니다. 기존 XML 기반 스캔을 시도합니다."
            )
    ...
    def _init_database(self):
        ...
        try:
            ...
            if "sibu_classification" not in columns:
                cursor.execute(
                    "ALTER TABLE book_analysis ADD COLUMN sibu_classification TEXT"
                )
    ...
    def extract_book_data(self, resource):
        ...
        try:
            ...
            if self.xlsx_mode and isinstance(resource, str):
                book_name = resource
                # XLSX에서 메트릭 생성: 현재는 P2S/S2P 결과 집계 위주로 구성
                p2s_df = None
                s2p_df = None
                try:
                    p2s_df = self._xlsx_load_p2s(book_name, self.base_dir)
                except Exception:
                    pass
                try:
                    s2p_df = self._xlsx_load_s2p(book_name, self.base_dir)
                except Exception:
                    pass

                # 간단한 집계 메트릭 (예: 평균 similarity, 결과 행 수 등)
                p2s_avg_similarity = (
                    float(p2s_df["similarity"].mean())
                    if p2s_df is not None
                    and "similarity" in pa_df.columns
                    and len(pa_df) > 0
                    else 0.0
                )
                ...
                # 작가/역자 정보 추출
                author, translator = self.metadata_extractor.extract_author_translator(
                    book_name
                )
            ...
            accuracy_file_in_folder = (
                result_folder / "accuracy" / "accuracy_report.json"
            )
            ...
            xml_similarity_file = (
                result_folder / "accuracy" / "xml_level_similarity.json"
            )
            pa_analysis = {}
            sa_analysis = {}
            ...
            # 4부분류 정보 추출
            sibu_classification = self.metadata_extractor.get_sibu_classification(
                book_name
            )
            ...
            book_data = {
                ...
                "pa_f1_score": pa_analysis.get("f1_score", 0.0),
                "pa_avg_similarity": pa_analysis.get("avg_similarity", 0.0),
                "pa_combined_similarity": pa_analysis.get(
                    "avg_combined_similarity", 0.0
                ),
                # SA 세부 메트릭
                ...
                "sa_target_similarity": sa_analysis.get(
                    "avg_translation_similarity", 0.0
                ),
            ...
            logger.info(
                f"{book_name} 데이터 추출 완료 - 품질등급: {book_data['quality_grade']}"
            )
            return book_data
    ...
    def store_analysis_data(self, book_data):
        ...
        try:
            ...
            # 기존 데이터 확인
            cursor.execute(
                "SELECT COUNT(*) FROM book_analysis WHERE book_name = ?",
                (book_data["book_name"],),
            )
    ...
    def generate_cumulative_report(self):
        ...
        try:
            ...
            df = pd.read_sql_query(
                "SELECT * FROM book_analysis ORDER BY analysis_date DESC", conn
            )
            ...
            report = {
                ...
                "average_embedding_similarity": float(
                    df["embedding_similarity_avg"].mean()
                ),
                ...
                book_summary = {
                    ...
                    "pa_details": {
                        "precision": (
                            ...
                            if pd.notna(row["pa_precision"])
                            else 0.0
                        ),
                        "recall": (
                            float(row["pa_recall"])
                            if pd.notna(row["pa_recall"])
                            else 0.0
                        ),
                        "f1_score": (
                            float(row["pa_f1_score"])
                            if pd.notna(row["pa_f1_score"])
                            else 0.0
                        ),
                        "avg_similarity": (
                            float(row["pa_avg_similarity"])
                            if pd.notna(row["pa_avg_similarity"])
                            else 0.0
                        ),
                        "combined_similarity": (
                            float(row["pa_combined_similarity"])
                            if pd.notna(row["pa_combined_similarity"])
                            else 0.0
                        ),
                    },
                    # SA 세부 메트릭
                    "sa_details": {
                        "precision": (
                            float(row["sa_precision"])
                            if pd.notna(row["sa_precision"])
                            else 0.0
                        ),
                        "recall": (
                            float(row["sa_recall"])
                            if pd.notna(row["sa_recall"])
                            else 0.0
                        ),
                        "f1_score": (
                            float(row["sa_f1_score"])
                            if pd.notna(row["sa_f1_score"])
                            else 0.0
                        ),
                        "set_similarity": (
                            float(row["sa_set_similarity"])
                            if pd.notna(row["sa_set_similarity"])
                            else 0.0
                        ),
                        "source_similarity": (
                            float(row["sa_source_similarity"])
                            if pd.notna(row["sa_source_similarity"])
                            else 0.0
                        ),
                        "target_similarity": (
                            float(row["sa_target_similarity"])
                            if pd.notna(row["sa_target_similarity"])
                            else 0.0
                        ),
                    },
                }
    ...
    def export_to_csv(self):
        ...
        try:
            ...
            if (
                Path(manual_csv_path).exists()
                and self.csv_export_path != "cumulative_analysis_results.csv"
            ):
    ...
    def merge_manual_csv(
        self, manual_csv_path: str = "cumulative_analysis_results_manual.csv"
    ):
        """수동 편집된 CSV와 새로운 데이터를 병합"""
        try:
            if not Path(manual_csv_path).exists():
                logger.warning(
                    f"수동 편집 CSV 파일이 존재하지 않습니다: {manual_csv_path}"
                )
                return False
        ...
        try:
            ...
            for _, row in manual_df.iterrows():
                ...
                book_data = {
                    ...
                    "total_paragraphs": (
                        int(row["총문단수"]) if pd.notna(row["총문단수"]) else 0
                    ),
                    "phrase_count": (
                        int(row["원문구수"]) if pd.notna(row["원문구수"]) else 0
                    ),
                    "sa_result_count": (
                        int(row["번역구수"]) if pd.notna(row["번역구수"]) else 0
                    ),
                    "length_accuracy": (
                        float(row["길이정확도(%)"]) / 100
                        if pd.notna(row["길이정확도(%)"])
                        else 0.0
                    ),
                    "global_source_similarity": (
                        float(row["원문유사도(%)"]) / 100
                        if pd.notna(row["원문유사도(%)"])
                        else 0.0
                    ),
                    "global_target_similarity": (
                        float(row["번역유사도(%)"]) / 100
                        if pd.notna(row["번역유사도(%)"])
                        else 0.0
                    ),
                    "pa_accuracy": (
                        float(row["PA정확도(%)"]) / 100
                        if pd.notna(row["PA정확도(%)"])
                        else 0.0
                    ),
                    "sa_accuracy": (
                        float(row["SA정확도(%)"]) / 100
                        if pd.notna(row["SA정확도(%)"])
                        else 0.0
                    ),
                    "embedding_similarity_avg": (
                        float(row["임베딩유사도(%)"]) / 100
                        if pd.notna(row["임베딩유사도(%)"])
                        else 0.0
                    ),
                    "pa_precision": (
                        float(row["PA_Precision(%)"]) / 100
                        if "PA_Precision(%)" in row and pd.notna(row["PA_Precision(%)"])
                        else 0.0
                    ),
                    "pa_recall": (
                        float(row["PA_Recall(%)"]) / 100
                        if "PA_Recall(%)" in row and pd.notna(row["PA_Recall(%)"])
                        else 0.0
                    ),
                    "pa_f1_score": (
                        float(row["PA_F1Score(%)"]) / 100
                        if "PA_F1Score(%)" in row and pd.notna(row["PA_F1Score(%)"])
                        else 0.0
                    ),
                    "pa_avg_similarity": (
                        float(row["PA_평균유사도(%)"]) / 100
                        if "PA_평균유사도(%)" in row
                        and pd.notna(row["PA_평균유사도(%)"])
                        else 0.0
                    ),
                    "sa_precision": (
                        float(row["SA_Precision(%)"]) / 100
                        if "SA_Precision(%)" in row and pd.notna(row["SA_Precision(%)"])
                        else 0.0
                    ),
                    "sa_recall": (
                        float(row["SA_Recall(%)"]) / 100
                        if "SA_Recall(%)" in row and pd.notna(row["SA_Recall(%)"])
                        else 0.0
                    ),
                    "sa_f1_score": (
                        float(row["SA_F1Score(%)"]) / 100
                        if "SA_F1Score(%)" in row and pd.notna(row["SA_F1Score(%)"])
                        else 0.0
                    ),
                    "sa_set_similarity": (
                        float(row["SA_한세트유사도(%)"]) / 100
                        if "SA_한세트유사도(%)" in row
                        and pd.notna(row["SA_한세트유사도(%)"])
                        else 0.0
                    ),
                    "sa_source_similarity": (
                        float(row["SA_원문유사도(%)"]) / 100
                        if "SA_원문유사도(%)" in row
                        and pd.notna(row["SA_원문유사도(%)"])
                        else 0.0
                    ),
                    "sa_target_similarity": (
                        float(row["SA_번역문유사도(%)"]) / 100
                        if "SA_번역문유사도(%)" in row
                        and pd.notna(row["SA_번역문유사도(%)"])
                        else 0.0
                    ),
                    "processing_time_seconds": (
                        float(row["처리시간(초)"])
                        if "처리시간(초)" in row and pd.notna(row["처리시간(초)"])
                        else 0.0
                    ),
                    "analysis_date": (
                        row["분석일시"]
                    ...
                    "sibu_classification": (
                        row["4부분류"]
                        if "4부분류" in row and pd.notna(row["4부분류"])
                        else self.metadata_extractor.get_sibu_classification(
                            row["책명"]
                        )
                    ),
                    "period": (
                        row["시대"]
                        if "시대" in row and pd.notna(row["시대"])
                        else self.metadata_extractor.get_period_from_author(
                            row["작가"]
                            if "작가" in row and pd.notna(row["작가"])
                            else "미상"
                        )
                    ),
                }
                ...
                if exists:
                    ...
                    logger.warning(
                        f"DB에 존재하지 않는 책: {book_data['book_name']} (수동 추가된 항목일 수 있음)"
                    )
    ...
    def run_full_analysis(self):
        ...
        try:
            ...
            if report:
                with open(
                    "cumulative_analysis_report.json", "w", encoding="utf-8"
                ) as f:
                    json.dump(report, f, ensure_ascii=False, indent=2)
                logger.info(
                    "누적 분석 보고서 저장 완료: cumulative_analysis_report.json"
                )
    ...
    try:
        ...
        if success:
            ...
            if Path(manual_csv_path).exists():
                ...
                print("\n💡 수동 편집 가이드:")
                print(
                    "   1. 수동 편집용 템플릿 생성: analyzer.create_manual_csv_template()"
                )
