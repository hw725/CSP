...
def init_worker(device_id=None, embedder_name='bge', openai_model=None, openai_api_key=None):
    ...
    try:
        ...
        print(f"워커 {mp.current_process().pid}: 기본 모듈 로드 완료")
            
    except Exception as e:
        print(f"워커 {mp.current_process().pid}: 기본 모듈 로드 실패: {e}")
        worker_modules = {}
    
    # 임베더 로드
    try:
        print(f"워커 {mp.current_process().pid}: 임베더 초기화 시작 ({embedder_name})")
        ...
        if worker_embed_func is None:
            ...
            print(f"워커 {mp.current_process().pid}: 임베더 로드 성공")
            
    except Exception as e:
        print(f"워커 {mp.current_process().pid}: 임베더 초기화 실패: {e}")
        worker_embed_func = None
        raise
