...
phrase_xlsx = pd.read_excel(
    "xlsx/당송팔대가문초구양수1/당송팔대가문초구양수1_구병렬.xlsx"
)
sent_xlsx = pd.read_excel(
    "xlsx/당송팔대가문초구양수1/당송팔대가문초구양수1_문장병렬.xlsx"
)
...
print(f"\n구병렬 xlsx:")
print(
    f'  문장식별자 범위: {phrase_xlsx["문장식별자"].min()} - {phrase_xlsx["문장식별자"].max()}'
)
...
print(f"\n문장병렬 xlsx:")
print(
    f'  문장식별자 범위: {sent_xlsx["문장식별자"].min()} - {sent_xlsx["문장식별자"].max()}'
)
...
if len(p1_xlsx) > 0:
    ...
    print(
        f"구병렬 xlsx 문장 1 ({len(p1_xlsx)} segments):\n  원문 합: {p1_full[:100]}..."
    )
...
if len(p76_xlsx) > 0:
    ...
    print(
        f"구병렬 xlsx 문장 76 ({len(p76_xlsx)} segments):\n  원문 합: {p76_full[:100]}..."
    )
