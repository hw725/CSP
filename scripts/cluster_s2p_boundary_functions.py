...
def cluster_kmeans(embeddings: np.ndarray, k: int, seed: int) -> np.ndarray:
    ...
    kmeans = MiniBatchKMeans(
        n_clusters=k, random_state=seed, batch_size=1024, n_init="auto"
    )
    labels = kmeans.fit_predict(embeddings)
    return labels

def save_results(
    instances: List[BoundaryInstance], labels: np.ndarray, out_dir: Path, seed: int
):
    import regex as re
...
def main():
    ...
    if args.load_embeddings and Path(args.load_embeddings).exists():
        ...
        texts = [
            inst.to_embed_text(use_src=args.use_src, use_tgt=args.use_tgt)
            for inst in boundaries
        ]
