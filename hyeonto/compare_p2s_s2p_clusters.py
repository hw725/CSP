...
def generate_comparison_html(p2s_stats: dict, s2p_stats: dict, out_path: Path):
    ...
    fig.add_trace(
        go.Bar(
            name=f"P2S ({p2s_stats['total_rows']:,})",
            x=all_markers,
            y=p2s_ratios,
    ...
    fig.add_trace(
        go.Bar(
            name=f"S2P ({s2p_stats['total_rows']:,})",
            x=all_markers,
            y=s2p_ratios,
            marker_color="coral",
        ),
        ...
        go.Table(
            ...
            cells=dict(
                values=[
                    ...
                    [
                        f"{p2s_stats['total_rows']:,}",
                        p2s_stats["num_clusters"],
                        p2s_stats.get("num_books", "N/A"),
                        p2s_stats["unique_markers"],
                        f"{p2s_stats.get('canon_ratio', 0):.1f}%",
                    ],
                    [
                        f"{s2p_stats['total_rows']:,}",
                        s2p_stats["num_clusters"],
                        s2p_stats.get("num_books", "N/A"),
                        s2p_stats["unique_markers"],
                        f"{s2p_stats.get('canon_ratio', 0):.1f}%",
                    ],
                ],
                font=dict(size=12),
            ),
...
def main():
    ...
    p.add_argument(
        ...
        type=Path,
        default=Path(
            "hyeonto/reports/boundary_function_clusters/boundary_clusters.csv"
        ),
    )
    p.add_argument(
        "--s2p-csv",
        type=Path,
        default=Path(
            "hyeonto/reports/phrase_boundary_clusters/s2p_boundary_clusters.csv"
        ),
    )
    p.add_argument(
        "--out-dir", type=Path, default=Path("hyeonto/reports/p2s_s2p_comparison")
    )
    ...
    print(
        f"P2S: {p2s_stats['total_rows']:,}개 경계, {p2s_stats['num_clusters']}개 클러스터"
    )
    ...
    print(
        f"S2P: {s2p_stats['total_rows']:,}개 경계, {s2p_stats['num_clusters']}개 클러스터"
    )
    ...
    generate_comparison_html(
        p2s_stats, s2p_stats, args.out_dir / "p2s_s2p_comparison.html"
    )

    # 마크다운 보고서
    generate_markdown_report(
        p2s_stats, s2p_stats, args.out_dir / "comparison_report.md"
    )
    ...
    summary = {
        ...
        "ratio_s2p_to_p2s": (
            s2p_stats["total_rows"] / p2s_stats["total_rows"]
            if p2s_stats["total_rows"] > 0
            else 0
        ),
    }
