...
# 복합 판별용 심 어미 (이것이 포함되고 길이가 3 이상이면 복합 가능성 높음)
...
def is_compound_marker(marker):
    ...
    # 심 복합 어미 포함 체크 (길이 3 이상, 단 정확히 일치하는 경우는 제외)
    if len(m) >= 3:
        for ce in COMPOUND_CORE_ENDINGS:
            # 심 어미 자체는 복합 아님 (예: '하여' 자체는 복합 아님)
            if m == ce:
                continue
            if ce in m:
                return True
...
def run_classification():
    """전체 마커 분류 실행"""
    df = pd.read_csv(DATA_DIR / "phrase_normalized_anonymized.csv")
