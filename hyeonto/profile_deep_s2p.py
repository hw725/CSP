...
def calculate_lift(
    df: pd.DataFrame, cluster_df: pd.DataFrame, min_count: int = 10
) -> list:
    """도서별 Lift 계산 (Top 5)"""
...
def analyze_cluster(cid: int, cluster_df: pd.DataFrame, full_df: pd.DataFrame) -> dict:
    ...
    # 1. 사서 비율
    is_canon = cluster_df["book_name"].apply(
        lambda x: any(b in str(x) for b in CANON_BOOKS)
    )
    ...
    mask = (
        cluster_df["src_left"].str.len() + cluster_df["src_right"].str.len()
    ).between(2, 50)
...
def generate_report(stats: list, total_rows: int, out_path: Path):
    ...
    for s in stats:
        ...
        lines.append(
            f"| p{s['id']} | {s['size']:,} | {s['canon_ratio']*100:.1f}% | {s['entropy']:.2f} | **{s['syntactic_guess']}** | {top_m} |"
        )

    lines.extend(["", "---", "", "## 2. 클러스터별 상세 분석", ""])
    ...
    for s in stats:
        ...
        lines.append(
            f"- **마커 다양성(Entropy)**: {s['entropy']:.2f} bits (Low=집중, High=다양)"
        )
