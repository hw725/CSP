...
4. 통계 분석 (analyze_hada_by_genre.py)
5. (선택) LLM 분석 (dansa_full_survey.py - API 키 필요)
6. 시각화 생성 (generate_*.py)
"""
...
def run_script(script_name: str, script_path: Path = None, args: list = None, 
               cwd: Path = None, skip_if_failed: bool = False) -> bool:
    """파이썬 스크립트 실행"""
...
def run_hyeonto_pipeline():
    ...
    steps = [
        ...
        ("phase4_premodern_classify.py", 
         SCRIPTS_DIR / "phase4_premodern_classify.py",
         ["--input", str(DATASETS_DIR / "phrase_normalized.csv")],
         False),  # 필수
        
        # Step 4: 장르별 분석 (하다 마커)
        ("analyze_hada_by_genre.py",
         SCRIPTS_DIR / "analyze_hada_by_genre.py",
         ["--input", str(DATASETS_DIR / "phrase_normalized.csv")],
         False),  # 필수
        
        # Step 5: LLM 분석 (선택, API 키 필요)
...
def run_visualizations():
    ...
    # 간단한 시각화부터 시작
    viz_scripts = [
        ("analyze_k4_clusters.py", None, False),
