            ...
            for tag, i1, i2, j1, j2 in sm.get_opcodes():
                if tag != 'equal':
                    ...
                    if tag == 'delete':
                        ...
                        logger.error(f"  변경됨: '{original_part}' → '{processed_part}' (위치 {i1}-{i2} → {j1}-{j2})")
            
            if not changes_found:
                logger.error(f"  차이점을 찾을 수 없음 (정규화 문제일 수 있음)")
                
            # 샘플 텍스트 출력 (처음 100자)
            logger.error(f"  원본 샘플: '{original[:100]}{'...' if len(original) > 100 else ''}'")
            logger.error(f"  처리 샘플: '{processed[:100]}{'...' if len(processed) > 100 else ''}'")
            
        except Exception as e:
            logger.error(f"차이점 분석 실패: {e}")
    
    def _log_restoration_differences(self, text_id: str, original: str, restored: str, corrupted: str):
        """복원 과정의 차이점 상세 로깅"""
        try:
            ...
            logger.error(f"변경 통계: 손상시 {corruption_changes}개, 복원시 {restoration_changes}개, 잔여 {remaining_changes}개")
    ...
    def restore_integrity(self, text_id: str, corrupted_parts: List[str], method: str = "auto") -> Tuple[List[str], bool]:
        ...
        try:
            ...
            if success:
                ...
                logger.error(f"  복원 길이 차이: {len(restored_normalized) - len(original_normalized)}자")
                
                # 복원 후에도 차이가 있는 부분 분석
                self._log_restoration_differences(text_id, original_normalized, restored_normalized, corrupted_normalized)
