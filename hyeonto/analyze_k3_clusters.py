...
def analyze_cluster_book_distribution(
    ...
    df["book_category"] = df["book"].apply(categorize_book)
    ...
    for cluster_id in sorted(df[cluster_col].unique()):
        ...
        book_dist = cluster_df["book"].value_counts().head(15)
...
def print_detailed_analysis(results: list, layer_name: str):
    ...
    print(f"? {layer_name} K=3 클러스터별 서종 분포 상세 분석")
    ...
    for r in results:
        ...
        print(
            f"    ? 삼경(三經): {r['samgyeong_count']:,}건 ({r['samgyeong_ratio']:.2f}%)"
        )
...
def generate_markdown_report(results: list, layer_name: str, output_path: Path):
    ...
    lines = [
        f"# {layer_name} K=3 클러스터별 서종(書種) 분포 상세 분석",
        "",
...
def main():
    ...
    if sentence_path.exists():
        ...
        output_sentence = (
            reports_dir / "sentence_k3_normalized" / "k3_book_distribution_analysis.md"
        )

        generate_markdown_report(
            sentence_results, "문장경계(Sentence)", output_sentence
        )
    ...
    if phrase_path.exists():
        ...
        output_phrase = (
            reports_dir / "phrase_k3_normalized" / "k3_book_distribution_analysis.md"
        )
        ...
        print(f"?? File not found: {phrase_path}")
