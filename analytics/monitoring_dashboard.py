...
class RealTimeDashboard:
                        ...
                        if 'pa_details' in book and 'sa_details' in book:
                            return {
                                'pa': {
                                    ...
                                    'avg_similarity': book['pa_details'].get('avg_similarity', 0),
                                    'combined_similarity': book['pa_details'].get('combined_similarity', 0)
                                },
                                'sa': {
                                    'precision': book['sa_details'].get('precision', 0),
                                    'recall': book['sa_details'].get('recall', 0),
                                    'f1_score': book['sa_details'].get('f1_score', 0),
                                    'set_similarity': book['sa_details'].get('set_similarity', 0),
                                    'source_only_similarity': book['sa_details'].get('source_similarity', 0),
                                    'target_only_similarity': book['sa_details'].get('target_similarity', 0)
                                }
                            }
            
            # 2. 백업: xml_pipeline_results 폴더에서 찾기 (기존 방식)
            json_path = f"../xml_pipeline_results/{book_name}/accuracy/accuracy_report.json"
            ...
            if 'xml_level_analysis' in json_data:
                ...
                if 'pa_analysis' in xml_analysis:
                    ...
                    details['pa'] = {
                        ...
                        'combined_similarity': pa_data.get('avg_combined_similarity', 0)
                    }
                ...
                if 'sa_analysis' in xml_analysis:
                    ...
                    details['sa'] = {
                        ...
                        'source_only_similarity': sa_data.get('avg_original_similarity', 0),
                        'target_only_similarity': sa_data.get('avg_translation_similarity', 0)
                    }
    ...
    def generate_dashboard(self):
        ...
        try:
            ...
            html_content = self._generate_html_content(stats, books, quality_dist, author_dist, sibu_dist, period_dist)
    ...
    def _generate_html_content(self, stats, books, quality_dist, author_dist, sibu_dist, period_dist):
        """HTML 콘텐츠 생성"""
        ...
        book_rows = ""
        for book in books:
            book_name, author, translator, grade, pa_acc, sa_acc, embed_sim, paragraphs, analysis_date, phrase_count, sa_result_count, global_source_sim, global_target_sim, length_acc, sibu_classification, period = book
            ...
            if '; ' in display_translator:
                ...
                if len(translator_parts) > 2:
                    translator_display = f"{translator_parts[0]} 외 {len(translator_parts)-1}명"
                else:
            ...
            if detail_metrics:
                ...
                detail_content = "<div class='no-data'>세부 분석 데이터를 찾을 수 없습니다.</div>"
        ...
        for period, count in period_dist[:10]:  # 상위 10개 시대만 표시
            period_display = period.split('(')[0] if '(' in period else period  # 괄호 내용 제거하여 간단히 표시
            period_stats += f"""
                <div class="performance-indicator">
        ...
        author_chart_data = []
        author_chart_labels = []
        author_colors = ['#FF6384', '#36A2EB', '#FFCE56', '#4BC0C0', '#9966FF', 
