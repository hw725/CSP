...
def process_single_file(
    ...
    **kwargs
    ...
    if embedder_name == 'openai':
        print("⚠️  OpenAI 임베더 사용 시 주의사항:")
        print("   - API 요청 제한으로 인해 처리 속도가 느릴 수 있습니다")
        print("   - 429 에러 발생 시 자동으로 재시도합니다")
        print("   - 워커 수가 자동으로 제한됩니다 (최대 2개)")
        print("   - 배치 크기가 작아집니다 (3문장씩)")
        print()
    ...
    try:
        if parallel:
            ...
            results_df = io_process_file(
                ...
                output_file,
                parallel=True,
                workers=workers,  # 파라미터 전달
                verbose=True,
                embedder_name=embedder_name,
                openai_model=openai_model if embedder_name == 'openai' else None,
                openai_api_key=openai_api_key if embedder_name == 'openai' else None
            )
