...
def process_batch_sentences(sentence_batch, device_id=None):
    ...
    try:
        if verbose_mode:
            print(f"🔄 워커 {mp.current_process().pid}: {batch_size}개 문장 배치 시작 (5문장 단위)")
        
        for idx, sentence_data in enumerate(sentence_batch):
            try:
                # 5문장 단위이므로 문장마다 또는 2문장마다 진행 상황 출력
                if batch_size <= 5:
                    # 5문장 이하: 모든 문장 또는 2문장마다
                    if (idx + 1) % 2 == 0 or (idx + 1) == batch_size:
                        print(f"🔄 워커 {mp.current_process().pid}: {idx+1}/{batch_size} 처리 중... ({((idx+1)/batch_size)*100:.0f}%)")
                else:
                    # 5문장 초과 (예외 상황): 3문장마다
                    if (idx + 1) % 3 == 0:
        ...
        # 처리 시간 기준을 더 관대하게 (문장당 2분)
        expected_time = batch_size * 120  # 문장당 2분 예상
        if processing_time > expected_time:
            print(f"⚠️ 워커 {mp.current_process().pid}: 배치 처리 시간 초과 ({processing_time:.1f}초 > {expected_time}초 예상)")
...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
    ...
    try:
        if parallel and total_sentences > workers:
            ...
            # 배치 크기를 5문장으로 고정 (핵심 변경사항)
            batch_size_per_worker = 5  # 모든 경우에 5문장으로 고정
            
            # 예외적으로 매우 작은 데이터셋의 경우만 조정
            if total_sentences <= 10:
                batch_size_per_worker = 2   # 10개 이하: 배치당 2개
            elif total_sentences <= 20:
                batch_size_per_worker = 3   # 20개 이하: 배치당 3개
            else:
                batch_size_per_worker = 5   # 기본: 배치당 5개
            ...
            print(f"   예상 배치당 처리 시간: {batch_size_per_worker * 120}초 (문장당 2분)")
            ...
            with mp.Pool(processes=workers, initializer=init_worker, 
                ...
                for batch_idx, async_result, batch_size in async_results:
                    try:
                        # 타임아웃을 600초로 고정
                        timeout_seconds = 600  # 10분 고정
                        
                        # 5문장 단위이므로 모든 배치 상태 출력
