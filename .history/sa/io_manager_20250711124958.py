...
def process_file(input_path: str, output_path: str, parallel: bool = False, workers: int = 4, 
    ...
    try:
        if parallel and total_sentences > workers:
            ...
            # OpenAI 사용 시 워커 수 제한 (API 제한 고려)
            if embedder_name == 'openai':
                workers = min(workers, 2)  # OpenAI는 최대 2워커만 사용
                print(f"🔧 OpenAI 사용으로 워커 수를 {workers}개로 제한")
            
            # 배치 크기도 OpenAI 사용 시 더 작게
            if embedder_name == 'openai':
                batch_size_per_worker = 3  # OpenAI는 배치당 3문장
            else:
                batch_size_per_worker = 5  # BGE는 배치당 5문장
            ...
            with mp.Pool(processes=workers, initializer=init_worker, 
                ...
                for batch_idx, async_result, batch_size in async_results:
                    try:
                        # OpenAI 사용 시 더 긴 타임아웃 설정
                        if embedder_name == 'openai':
                            timeout_seconds = max(600, batch_size * 120)  # OpenAI: 문장당 2분
                        else:
                            timeout_seconds = max(300, batch_size * 60)   # BGE: 문장당 1분
                        
                        print(f"⏳ 배치 {batch_idx+1}/{len(sentence_batches)} 처리 중... (임베더: {embedder_name}, 크기: {batch_size}, 타임아웃: {timeout_seconds}초)")
                        ...
                        error_msg = f"배치 {batch_idx+1} 타임아웃 (임베더: {embedder_name}, 크기: {batch_size}, 제한: {timeout_seconds}초)"
