...
class SikuBertTokenizer:
    ...
    def __init__(self, model_name: str = "SIKU-BERT/sikubert"):
        ...
        if torch.cuda.is_available():
            ...
            if not os.getenv("CSP_QUIET_INIT"):
                print(
                    f"⚠️ SikuBERT: GPU 불가능, CPU 모드로 실행 (device: {self.device})"
                )
    ...
    def load_model(self):
        ...
        try:
            ...
            error_msg = str(e)
            if (
                "torch.load" in error_msg
                or "vulnerability" in error_msg
                or "CVE-2025-32434" in error_msg
            ):
                logger.error(
                    f"SikuBERT 모델 로딩 실패 (PyTorch 보안 문제): torch.load 함수가 보안상 제한됨"
                )
                logger.error(
                    "해결책: PyTorch 2.6+ 업그레이드 또는 safetensors 기반 모델 사용"
                )
            else:
...
def siku_get_embeddings(
    texts: List[str], max_length: int = 512, batch_size: int = 32
) -> np.ndarray:
    """
