...
사용 예:
    python hyeonto/scripts/analyze_hanja_marker_cooccurrence.py \
...
def extract_hanja_marker_pairs(
    text: object, use_distance_weight: bool = True
...
def main() -> int:
    ...
    p.add_argument(
        "--out-dir", type=Path, default=Path("hyeonto/reports/hanja_marker_analysis")
    )
    ...
    # 1. (한자, 현토, 가중치) 삼중쌍 수집
    hanja_marker_counts: dict[str, dict[str, float]] = defaultdict(
    ...
    try:
        ...
        polysemy_markers = [r for r in polysemy_results if r["polysemy"] == "⚠️다의적"][
            :5
        ]
        ...
        if polysemy_markers:
            ...
            for pm in polysemy_markers:
                ...
                kmeans = KMeans(
                    n_clusters=pm["optimal_k"], random_state=args.seed, n_init=10
                )
                ...
                for cl in range(pm["optimal_k"]):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="markers+text",
                            marker=dict(
                                size=np.clip(np.log1p(cl_counts) * 3, 6, 20),
                ...
                fig.write_html(
                    str(args.out_dir / f"polysemy_{marker}.html"),
    ...
    cfg = {
        ...
        "polysemy_detected": sum(
            1 for r in polysemy_results if r["polysemy"] == "⚠️다의적"
        ),
    }
