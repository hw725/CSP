...
def extract_gold_subset(
    ...
    print(f"✅ gold subset 저장: {out_path} (rows={len(subset)})")
    ...
    print(f"✅ gold paragraphs 저장: {out_paragraph_path} (rows={len(para_df)})")
    ...
    if sa_gold_path is not None:
        ...
        print(f"✅ SA gold(구병렬) subset 저장: {out_sa_path} (rows={len(sa_subset)})")
...
def run_pa_output_vs_gold_report(
    ...
    print(f"✅ 번역문 문장리스트 완전일치: {len(tgt_exact_ok)}/{total_paras}")
    # 신규 출력: 문장 단위
    if tgt_sent_total_gold > 0:
        print(f"✅ 번역문 문장 완전일치(문장): {tgt_sent_exact_ok}/{tgt_sent_total_gold}")
    ...
    if tgt_exact_fail:
        if pred_has_book:
            ...
            print(f"❌ 번역문 불일치 문단: {pretty}")
        else:
            print(f"❌ 번역문 불일치 문단: {tgt_exact_fail}")
...
def run_pa_output_integrity_report(pa_output: Path, source_paragraphs: Path) -> int:
    ...
    print()
    print(f"✅ 전역 결합 무결성(원문, 공백정규화): {'PASS' if global_src_ok else 'FAIL'}")
    print(f"✅ 전역 결합 무결성(번역문, 공백정규화): {'PASS' if global_tgt_ok else 'FAIL'}")
    ...
    print()
    print(f"✅ 문단별 결합 무결성(원문) 통과: {len(ok_concat_src)}/{total_paras}")
    print(f"✅ 문단별 결합 무결성(번역문) 통과: {len(ok_concat_tgt)}/{total_paras}")
    print(f"✅ 문단별 순서 보존(원문) 통과: {len(ok_order_src)}/{total_paras}")
    print(f"✅ 문단별 순서 보존(번역문) 통과: {len(ok_order_tgt)}/{total_paras}")
    print(f"✅ 번역문 분할(문장 수/순서) 완전일치: {len(ok_tgt_split_exact)}/{total_paras}")
