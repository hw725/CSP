...
def add_sibu_to_manual_csv():
    ...
    manual_df = pd.read_csv(
        "cumulative_analysis_results_manual.csv", encoding="utf-8-sig"
    )
    ...
    # 원본 파일을 백업
    manual_df_original = pd.read_csv(
        "cumulative_analysis_results_manual.csv", encoding="utf-8-sig"
    )
    manual_df_original.to_csv(
        "cumulative_analysis_results_manual_backup.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    manual_df.to_csv(
        "cumulative_analysis_results_manual.csv", index=False, encoding="utf-8-sig"
    )
