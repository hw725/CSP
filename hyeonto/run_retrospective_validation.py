...
# 회상상 마커 패턴
RETROSPECTIVE_PATTERNS = [r'러(?=[ㄱ-ㅎㅏ-ㅣ가-힣])', r'려(?=[ㄱ-ㅎㅏ-ㅣ가-힣])']
OTHER_TAM_PATTERNS = {
    'retrospective': [r'더(?=[ㄱ-ㅎㅏ-ㅣ가-힣])'],
    'modal': [r'리(?=[ㄱ-ㅎㅏ-ㅣ가-힣])'],
}
...
def test_h1_retrospective_identity(df: pd.DataFrame, n_perm: int = 1000) -> dict:
    ...
    print(f"  [반대가설] {'✅ 강건' if result['inverse_verdict'] == 'ROBUST' else '⚠️ 민감'}: 기대 대비 {result['robustness']:.2f}x")
    print(f"  [대립가설] {'✅ 우수' if result['alt_verdict'] == 'SUPERIOR' else '⚠️ 동등'}: 랜덤 대비 {result['alt_ratio']:.2f}x")
...
def test_h2_1_genre_distribution(df: pd.DataFrame, n_perm: int = 1000) -> dict:
    ...
    print(f"  [반대가설] {'✅ 강건' if result['inverse_verdict'] == 'ROBUST' else '⚠️ 민감'}")
    print(f"  [대립가설] {'✅ 우수' if result['alt_verdict'] == 'SUPERIOR' else '⚠️ 동등'}")
    print(f"\n  🔹 종합: {result['passed_count']}/3 통과")
...
def save_report(results: dict, output_dir: Path):
    ...
    for h_id, h_name, h_desc, h_data in hypotheses:
        ...
        md_lines.extend([
            ...
            f"| 반대가설 | {'✅ 강건' if h_data['inverse_verdict'] == 'ROBUST' else '⚠️ 민감'} | 기대 대비 {h_data['robustness']:.2f}x |",
            f"| 대립가설 | {'✅ 우수' if h_data['alt_verdict'] == 'SUPERIOR' else '⚠️ 동등'} | 랜덤 대비 {h_data['alt_ratio']:.2f}x |",
            "",
        ])
