...
class CrossAttnBoundaryTaggerLoader:
    ...
    def __init__(self, model_path: Path = None, device: str = "cuda"):
        ...
        if model_path is None:
            ...
            model_path = (
                Path(__file__).parent.parent / "models" / "s2p_crossattn_boundary.pt"
            )
        ...
        # 체크포인트 키 호환성
        state_dict_key = (
            "state_dict" if "state_dict" in checkpoint else "model_state_dict"
        )
        ...
        print(
            f"✅ Cross-Attention 경계 태거 로드 완료 (src_vocab={len(self.src_vocab)}, tgt_vocab={len(self.tgt_vocab)}, device={self.device})"
        )
    ...
    def _encode_batch(
        self, texts: List[str], vocab: Dict, max_len: int
    ) -> torch.Tensor:
    ...
    def segment_text_batch(
        ...
        **kwargs,
        ...
        with torch.no_grad():
            ...
            # 배치 순방향 추론
            logits, attn_weights = self.model.forward_with_attention(
                src_batch, tgt_batch
            )
        ...
        for i in range(batch_size):
            ...
            if n_segments is not None and n_segments > 1:
                ...
                top_positions = sorted(
                    [pos for _, pos in prob_positions[: n_segments - 1]]
                )
