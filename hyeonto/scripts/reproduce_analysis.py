...
사용법:
  python hyeonto/scripts/reproduce_analysis.py
"""
...
def analyze_level1(df: pd.DataFrame) -> dict:
    ...
    # 카이제곱 검정
    contingency = [[roda_pos, n_roda - roda_pos], [ctrl_pos, n_ctrl - ctrl_pos]]
...
def analyze_level2(df: pd.DataFrame) -> dict:
    ...
    # 카이제곱 검정
    contingency = [[nira_pos, n_nira - nira_pos], [ra_pos, n_ra - ra_pos]]
    chi2, p_value, dof, expected = chi2_contingency(contingency)
...
def main():
    ...
    print(f"{'='*60}")
    print(
        f"  로다: {l1_stats['roda_positive']:,}/{l1_stats['n_roda']:,} ({l1_stats['roda_rate']:.1f}%)"
    )
    print(
        f"  대조군: {l1_stats['control_positive']:,}/{l1_stats['n_control']:,} ({l1_stats['control_rate']:.1f}%)"
    )
    ...
    print(f"{'='*60}")
    print(
        f"  니라: {l2_stats['nira_positive']:,}/{l2_stats['n_nira']:,} ({l2_stats['nira_rate']:.1f}%)"
    )
    print(
        f"  라: {l2_stats['ra_positive']:,}/{l2_stats['n_ra']:,} ({l2_stats['ra_rate']:.1f}%)"
    )
