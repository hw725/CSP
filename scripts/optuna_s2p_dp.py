#!/usr/bin/env python3
"""S2P DP Alignment 파라미터 Optuna 튜닝
...
def main():
    ...
    print("=" * 60)
    print("🎯 S2P DP Alignment Optuna 튜닝")
    print("=" * 60)
    ...
    def callback(study, trial):
        print(
            f"  Trial {trial.number}: F1={trial.value:.4f} | best={study.best_value:.4f}"
        )
    ...
    out_path = Path("s2p_dp_optuna_result.json")
    legacy_path = Path("sa_dp_optuna_result.json")
    ...
    print("\n💾 Saved: s2p_dp_optuna_result.json (legacy: sa_dp_optuna_result.json)")
