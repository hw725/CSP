...
def generate_3d_overlay_html(csv_path: Path, output_path: Path):
    ...
    if "z" not in df.columns:
        ...
        df["z"] = df["cluster_id"].map(cluster_z_base) + np.random.normal(
            0, 0.5, len(df)
        )
...
def main():
    ...
    csv_path = reports_dir / "k3_embedding_overlay_2d.csv"

    output_path = reports_dir / "k3_embedding_overlay_3d.html"
