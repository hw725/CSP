...
def merge_and_save_sentence(
    source_data, translation_data, output_excel_path, book_name
):
    ...
    print(
        f"✓ 문장병렬 Excel 생성: {output_excel_path.name} ({len(rows)}개 행, {len(unique_para_ids)}개 고유 문단식별자)"
    )
...
def process_xml_pairs(source_dir, output_base_dir):
    ...
    for source_xml in source_files:
        ...
        try:
            ...
            # Excel로 저장
            row_count = merge_and_save_sentence(
