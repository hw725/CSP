...
def evaluate_p2s_output(pred_path: Path, gold_path: Path) -> float:
    ...
    for key in common_keys:
        ...
        pred_bounds = {
            (key, p) for p in _boundary_positions_normed(pred_rows["원문"].tolist())
        }
        gold_bounds = {
            (key, p) for p in _boundary_positions_normed(gold_rows["원문"].tolist())
        }
...
def objective(
    trial: optuna.Trial, input_path: str, gold_path: str, output_dir: Path
) -> float:
    ...
    shift_penalty_factor = trial.suggest_float(
        "shift_penalty_factor", 0.0001, 0.002, log=True
    )
    ...
    print(
        f"[Trial {trial.number}] bt={boundary_threshold:.3f} bf={boundary_bonus_factor:.2f} sp={shift_penalty_factor:.5f} → F1={f1:.4f}"
    )
