...
def process_merged_xml(files):
    ...
    for filepath in files:
        ...
        try:
            ...
            original_count = len([elem for elem in root.iter() if elem.tag in ['원문', 'original']])
            translation_count = len([elem for elem in root.iter() if elem.tag in ['번역문', 'translation']])
...
예제:
  ...
  python main.py xlsx process file.xlsx
    ...
    # XML 서브커맨드
    xml_parser = subparsers.add_parser('xml', help='XML 파일 처리 (분리된 원문/번역문 쌍)')
    ...
    xml_process_parser.add_argument('--original', required=True, help='원문 XML 파일 경로')
    xml_process_parser.add_argument('--translation', required=True, help='번역문 XML 파일 경로')
    ...
    xml_merged_parser = subparsers.add_parser('xml-merged', help='통합 XML 파일 처리 (원문+번역문 통합)')
    xml_merged_parser.add_argument('merged_command', help='통합 XML 명령어 (process, split, analyze)')
    xml_merged_parser.add_argument('merged_args', nargs='*', help='통합 XML 명령어 인수들')
    ...
    if args.format == 'xml':
        ...
        if args.xml_command == 'process':
            sys.argv = ['xml_pipeline_cli.py', 'process', 
        ...
        if 'xml' in unique_types and len(unique_types) == 1:
            ...
            if len(xml_files) == 1:
                ...
                if xml_type == 'merged':
                    print("🎯 통합 XML 파이프라인으로 처리합니다 (원문+번역문 통합 파일).")
                    ...
                    if pair_file:
                        ...
                        from xml_pipeline.xml_pipeline_cli import main as xml_main
                        sys.argv = ['xml_pipeline_cli.py', 'process', '--original', first_xml, '--translation', pair_file]
                        xml_main()
                    else:
                        print("🎯 분리 XML 스마트 모드로 처리합니다 (디렉토리 전체 스캔).")
                        from xml_pipeline.xml_pipeline_cli import main as xml_main
                        sys.argv = ['xml_pipeline_cli.py', 'smart', '--xml-dir', str(Path(first_xml).parent)]
                        xml_main()
            else:
                # 여러 XML 파일인 경우
                print("🎯 분리 XML 파이프라인으로 처리합니다 (여러 파일).")
                from xml_pipeline.xml_pipeline_cli import main as xml_main
                sys.argv = ['xml_pipeline_cli.py', 'smart', '--xml-dir', str(Path(xml_files[0]).parent)]
                xml_main()
        else:
