...
## SA (Sentence Aligner) 모듈 구조 및 워크플로우 - 의미 기반 (2025-12-14 최신)

### 주요 업데이트 (2025-12-14)
- ✅ **토큰 단위 보존**: 한자 괄호 augmentation이 토큰 개수를 유지하도록 개선
- 🔒 **출력 크기 보장**: N개 항목 반환 보장으로 번역문 손실/중복 방지
- 📊 **유사도 점수 통합**: 코사인 유사도 열 추가 (PA와 동일)
...
flowchart TB
    ...
    %% 공통 토크나이저 의존성
    CT[📁 ../common/tokenizers/<br/>하이브리드 토크나이저<br/>SikuBERT+RoBERTa-Hanja+Kiwipiepy]:::tokenizer
...
flowchart TD
    ...
    %% 🆕 하이브리드 토크나이저 초기화  
    HT[🏮 하이브리드 토크나이저 초기화<br/>SikuBERT+RoBERTa-Hanja+Kiwipiepy<br/>+ BGE-M3 FlagModel]:::process
...
flowchart TD
    ...
    O[🏮 하이브리드 토크나이징<br/>SikuBERT + RoBERTa-Hanja+Kiwipiepy]:::process
...
flowchart TD
    ...
    %% 하이브리드 토크나이저 초기화
    E[🏮 하이브리드 토크나이저 설정<br/>SikuBERT+RoBERTa-Hanja+Kiwipiepy]:::tokenizer
    ...
    %% 토크나이징 단계
    I[✂️ 원문 토크나이징<br/>SikuBERT 우선 + Kiwipiepy 하이브리드]:::tokenizer
...
flowchart TB
    ...
    %% 개별 처리 과정
    G[🎭 구두점 마스킹]:::process
    H[🏮 하이브리드 토크나이징<br/>SikuBERT + RoBERTa-Hanja+Kiwipiepy]:::process
    I[🧠 임베딩]:::process
    J[📐 정렬]:::process
    K[🎭 구두점 복원]:::process
