        ...
        if result.returncode == 0:
            ...
            if p2s_gt_file:
                ...
                try:
                    eval_cmd = [
                        "python",
                        "accuracy/accuracy_evaluator.py",
                        str(p2s_gt_file),
                        str(p2s_output),
                        "--project",
                        "p2s",
                    ...
                    eval_result = subprocess.run(eval_cmd, capture_output=True)
                    if eval_result.returncode == 0:
                        ...
                        if eval_result.stderr:
                            print(
                                "    ",
                                eval_result.stderr.decode(errors="ignore").strip(),
                            )
...
try:
    result = subprocess.run(
        [
            "python",
            "analytics/aggregate_batch_results.py",
            "--results-dir",
            str(output_dir),
            "--output-dir",
            "analytics",
