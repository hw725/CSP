...
def evaluate_s2p(gold_file, pred_file, sample_size=None):
    ...
    for _, row in gold_df.iterrows():
        ...
        gold_grouped[sent_id].append(
            {"src": normalize_text(row["원문"]), "tgt": normalize_text(row["번역문"])}
        )
    ...
    for sent_id in common_ids:
        ...
        results["src_similarity"].append(
            calculate_similarity(gold_src_full, pred_src_full)
        )

        # 번역문 비교
        tgt_exact = gold_tgt_full == pred_tgt_full
        results["tgt_exact_match"].append(1.0 if tgt_exact else 0.0)
        results["tgt_similarity"].append(
            calculate_similarity(gold_tgt_full, pred_tgt_full)
        )
    ...
    print(f"\n📌 데이터 커버리지:")
    print(
        f"  평가된 문장 수: {len(common_ids)} / {len(gold_grouped)} ({100*len(common_ids)/len(gold_grouped):.1f}%)"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--output",
        "-o",
        default="test_results/s2p_quick_eval.json",
        help="결과 저장 경로",
    )
