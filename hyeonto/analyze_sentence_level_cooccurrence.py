...
def find_optimal_k(
    X: np.ndarray, max_k: int = 5, min_samples: int = 10
) -> tuple[int, float]:
...
def main() -> int:
    ...
    p.add_argument(
        "--out-dir", type=Path, default=Path("hyeonto/reports/sentence_level_analysis")
    )
    ...
    # 공기 카운트 (전체 / 종결 / 비종결)
    hanja_marker_all: dict[str, dict[str, float]] = defaultdict(
        lambda: defaultdict(float)
    )
    hanja_marker_final: dict[str, dict[str, float]] = defaultdict(
        lambda: defaultdict(float)
    )
    hanja_marker_nonfinal: dict[str, dict[str, float]] = defaultdict(
        lambda: defaultdict(float)
    )
    ...
    # 2. 필터링
    top_markers = sorted(marker_total.keys(), key=lambda m: -marker_total[m])[
        : args.top_markers
    ]
    ...
    for h in hanja_list:
        for m in marker_list:
            ...
            C_nonfinal[hanja_to_idx[h], marker_to_idx[m]] = hanja_marker_nonfinal[
                h
            ].get(m, 0)
    ...
    for m_idx, marker in enumerate(marker_list):
        ...
        # 종결/비종결 각각의 한자 분포 비교
        final_hanja = [
            (h, C_final[hanja_to_idx[h], m_idx])
            for h in hanja_list
            if C_final[hanja_to_idx[h], m_idx] > 0
        ]
        nonfinal_hanja = [
            (h, C_nonfinal[hanja_to_idx[h], m_idx])
            for h in hanja_list
            if C_nonfinal[hanja_to_idx[h], m_idx] > 0
        ]
    ...
    for m_idx, marker in enumerate(marker_list):
        hanja_counts = [
            (hanja_to_idx[h], h, C[hanja_to_idx[h], m_idx])
            for h in hanja_list
            if C[hanja_to_idx[h], m_idx] > 0
        ]
    ...
    func_df.to_csv(
        args.out_dir / "marker_function_analysis.csv", index=False, encoding="utf-8-sig"
    )
    ...
    poly_df.to_csv(
        args.out_dir / "marker_polysemy_analysis.csv", index=False, encoding="utf-8-sig"
    )
    ...
    try:
        ...
        colors = [
            "#d62728" if r < 0.3 else ("#2ca02c" if r > 0.7 else "#ff7f0e")
        ...
        fig.write_html(
            str(args.out_dir / "marker_final_ratio.html"), include_plotlyjs="cdn"
        )
    ...
    for r in sorted(polysemy_results, key=lambda x: -x["silhouette"])[:10]:
        print(
            f"  {r['marker']:>6} : K={r['optimal_k']}, Sil={r['silhouette']:.2f}, {r['polysemy']}"
        )
