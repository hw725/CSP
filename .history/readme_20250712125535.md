...
한문-한국어 번역 텍스트의 자동 정렬을 위한 경량화된 CLI 도구 (SA/PA)
...
- **SA (Sentence/Phrase Alignment):** 문장 및 구 단위 정렬을 지원합니다.
- **PA (Paragraph-to-Sentence Alignment):** 문단 단위의 텍스트를 문장으로 분할하고 정렬합니다.
- **모듈화된 구조:** `sa` 와 `pa` 디렉토리에 각 기능이 분리되어 있으며, 공통 모듈을 공유합니다.
- **지원 임베더:**
  - `BGE-M3`: 로컬에서 실행되는 고성능 임베딩 모델입니다.
  - `OpenAI`: `text-embedding-3-large` 등 최신 모델을 API 키와 함께 사용할 수 있습니다.
- **최적화 및 경량화:**
  - `huggingface/transformers`, `sentence-transformers` 등 무거운 라이브러리를 제거하여 경량화했습니다.
  - 대용량 데이터 처리에 최적화된 CLI 인터페이스를 제공합니다.
  - 실시간 진행률 표시(tqdm), 임베딩 캐시, 상세 로그 기능을 지원합니다.
...
1.  **Poetry 설치**
    Poetry가 없다면 먼저 설치합니다. ([공식 문서](https://python-poetry.org/docs/#installation) 참고)
    ```bash
    pip install poetry
    ```

2.  **의존성 설치**
    프로젝트 루트에서 `poetry install`을 실행하면 `pyproject.toml` 기반으로 가상환경 생성과 패키지 설치가 진행됩니다.
    ```bash
    poetry install
    ```

3.  **Poetry 쉘 활성화**
    작업을 위해 Poetry가 만든 가상환경으로 진입합니다.
    ```bash
    poetry shell
    ```

4.  **PyTorch 설치 (BGE 임베더 사용 시)**
    `poetry shell`로 진입한 상태에서 환경에 맞는 PyTorch를 설치합니다.
    ...
    - **CPU 버전:**
      ...
      ```
    - **CUDA (NVIDIA GPU) 버전:**
      (사용자의 CUDA 버전에 맞는 명령을 [PyTorch 공식 홈페이지](https://pytorch.org/get-started/locally/)에서 확인하세요.)
      ```bash
      # 예시 (CUDA 12.1)
      pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu121
      ```

5.  **spaCy 모델 설치 (PA 기능 사용 시)**
    `poetry shell`로 진입한 상태에서 PA 기능에 필요한 spaCy 모델을 설치합니다.
    ```bash
    python -m spacy download ko_core_news_lg
    ```
...
**poetry shell로 활성화된 가상환경에서 아래와 같이 실행합니다.**
...
- **SA (문장/구 정렬) 예시:**
  ...
  # OpenAI 임베더 사용
  python sa/main.py input.xlsx output.xlsx --embedder openai --openai-model text-embedding-3-large --openai-api-key "YOUR_API_KEY"
  ```
...
- **입력:** `.xlsx` (Excel) 파일
  - **PA:** `문단(원문)`, `문단(번역문)` 2개 컬럼
  - **SA:** `원문(샘플)`, `번역문(샘플)` 2개 컬럼
- **출력:** `.xlsx` (Excel) 파일
  - 정렬된 문장/구가 원본 텍스트와 함께 저장됩니다.
>>>>>>> bff285a (docs: Update README for current stable version)
