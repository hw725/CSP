...
def log_current_configuration():
    logger.info(
        f"⚙️ KANBUN_FORCE={KANBUN_FORCE}, KANBUN_AVAILABLE={KANBUN_AVAILABLE}, STANZA_MODULE_AVAILABLE={STANZA_MODULE_AVAILABLE}"
    )
    if KANBUN_AVAILABLE:
...
def ensure_kanbun_pipeline(BERT: str | None = None, Danku: bool | None = None):
    ...
    if not KANBUN_AVAILABLE:
        ...
        try:
            logger.warning(
                "⚠️ Kanbun 파서 미설치: suparkanbun/esupar 설치를 시도합니다..."
            )
...
def ensure_stanza_pipeline(lang: str = "ko"):
    ...
    if not STANZA_MODULE_AVAILABLE:
        ...
        try:
            ...
            subprocess.check_call(
                [sys.executable, "-m", "pip", "install", "--no-cache-dir", "stanza"]
            )
            import importlib
            ...
            raise RuntimeError(
                f"Stanza가 설치되지 않았습니다. 자동 설치도 실패했습니다: {e}"
            )
    ...
    def _try_init() -> "stanza.Pipeline":
        return stanza.Pipeline(
            lang, model_dir=model_dir, use_gpu=use_gpu, verbose=False
        )
    ...
    try:
        ...
        logger.info(
            f"✅ Stanza 파이프라인 초기화 완료 (lang={lang}, GPU={use_gpu}, dir={model_dir})"
        )
        return stanza_nlp
    except Exception as e:
        ...
        try:
            logger.warning(
                f"⚠️ Stanza 파이프라인 초기화 실패: {e} → 리소스 다운로드 후 재시도"
            )
            stanza.download(lang, model_dir=model_dir, verbose=False)
            stanza_nlp = _try_init()
            STANZA_AVAILABLE = True
            logger.info(
                f"✅ Stanza 파이프라인 초기화 완료(재시도) (lang={lang}, GPU={use_gpu}, dir={model_dir})"
            )
            return stanza_nlp
            ...
            if hasattr(doc, "sentences") and doc.sentences:
                for s in doc.sentences:
                    if hasattr(s, "values") and s.values:
                        sent_text = "".join(
                            [getattr(tok, "form", "") for tok in s.values]
                        )
                        if sent_text.strip():
    ...
    try:
        ...
        for sent in doc.sentences:
            for token in sent.tokens:
                ...
                if upos in {"SCONJ", "CCONJ"} or (
                    deprel in {"mark", "cc", "conj", "advcl", "parataxis"}
                ):
                    if token.end_char is not None:
                        offsets.append(token.end_char - 1)
        # 정렬·중복 제거
...
def get_chinese_unit_boundary_indices_supar(
    source_text: str, source_units: list[str]
) -> set[int]:
    ...
    try:
        ...
        try:
            for s in getattr(parsed, "sentences", []) or []:
                if hasattr(s, "values") and s.values:
                    sent_text = "".join(
                        [tok.form for tok in s.values if hasattr(tok, "form")]
                    )
                    if sent_text:
