#!/usr/bin/env python3
"""
SA 클러스터 결과를 대립 가설 검증용으로 준비

1. 현토 마커 추출 (src_left, src_right에서) - 구 단위
...
# src_left와 src_right에서 마커 추출 (구 단위이므로 보통 1개씩)
...
def classify_syntactic_function(marker):
    ...
    # 매핑 테이블 조회
    if marker in syntactic_mapping:
        return syntactic_mapping[marker]

    # 규칙 기반
    if "?" in marker or marker in ["니", "가", "ㄴ가", "뇨", "ㄹ까", "ㄴ저"]:
        return "의문종결"
    elif marker in ["라", "니라", "도다", "리라", "러라"]:
        return "평서종결"
    else:
        return "기타"
...
for cluster_id in sorted(df["cluster_id"].unique()):
    ...
    print(
        f"  Cluster {cluster_id}: {saseo_ratio:.1f}% ({saseo_count}/{len(cluster_df)})"
    )
