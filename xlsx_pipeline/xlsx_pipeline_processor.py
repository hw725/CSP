        ...
        if self._word_df is None or force_reload:
            if not self.word_parallel_path.exists():
                raise FileNotFoundError(
                    f"구병렬 파일을 찾을 수 없습니다: {self.word_parallel_path}"
                )
    ...
    def load_sentence_parallel(self, force_reload: bool = False) -> pd.DataFrame:
        ...
        if self._sentence_df is None or force_reload:
            if not self.sentence_parallel_path.exists():
                raise FileNotFoundError(
                    f"문장병렬 파일을 찾을 수 없습니다: {self.sentence_parallel_path}"
                )

            df = pd.read_excel(self.sentence_parallel_path)
            self._sentence_df = normalize_dataframe(df)
            logger.info(
                f"✅ 문장병렬 로드 완료: {len(self._sentence_df)}행 (정규화 적용)"
            )
    ...
    def load_paragraph_parallel(self, force_reload: bool = False) -> pd.DataFrame:
        ...
        if self._paragraph_df is None or force_reload:
            if not self.paragraph_parallel_path.exists():
                raise FileNotFoundError(
                    f"문단병렬 파일을 찾을 수 없습니다: {self.paragraph_parallel_path}"
                )

            df = pd.read_excel(self.paragraph_parallel_path)
            self._paragraph_df = normalize_dataframe(df)
            logger.info(
                f"✅ 문단병렬 로드 완료: {len(self._paragraph_df)}행 (정규화 적용)"
            )
...
class XLSXPipelineProcessor:
    ...
    def discover_books(self) -> List[str]:
        ...
        if not self.xlsx_root_dir.exists():
            logger.error(
                f"❌ XLSX 루트 디렉토리를 찾을 수 없습니다: {self.xlsx_root_dir}"
            )
            return []
    ...
    def get_all_statistics(self) -> Dict[str, Any]:
        ...
        for book_id, book in self.books.items():
            try:
                ...
                all_stats["books"][book_id] = {"book_id": book_id, "error": str(e)}
    ...
    def process_book_pipeline(
        self, book_id: str, config: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
        특정 책에 대한 파이프라인 실행
        ...
        if self.analyzer and any(
            a in analysis for a in ["pa", "sa", "accuracy", "full"]
        ):
            try:
                # full 분석 실행
                if "full" in analysis:
                    if (
                        word_df is not None
                        and sent_df is not None
                        and para_df is not None
                    ):
                    ...
                    if "accuracy" in analysis:
                        logger.warning(
                            "⚠️ accuracy 분석은 'full' 모드에서만 실행됩니다"
                        )
