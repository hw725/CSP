...
class SemanticBoundaryModel(nn.Module):
    ...
    def forward(self, src_embs, tgt_ids, n_phrases):
        ...
        phrase_mask = torch.arange(max_phrases, device=src_embs.device).unsqueeze(
            0
        ) >= n_phrases.unsqueeze(1)
...
class SemanticBoundaryTaggerLoader:
    ...
    def __init__(self, model_path: Path = None, device: str = "cuda"):
        ...
        if model_path is None:
            candidates = [
                Path(__file__).parent.parent / "models" / "s2p_semantic_boundary.pt",
                Path(__file__).parent.parent / "models" / "sa_semantic_boundary.pt",
            ]
        ...
        if not model_path.exists():
            ...
            raise FileNotFoundError(f"의미 대응 경계 모델 없음: {model_path} (expected one of: {hint})")
        ...
        print(
            f"✅ 의미 대응 경계 태거 로드 완료 (tgt_vocab={len(self.tgt_vocab)}, device={self.device})"
        )
    ...
    def segment_text(
        ...
        n_phrases_tensor = torch.tensor(
            [n_phrases], dtype=torch.long, device=self.device
        )
...
def get_semantic_boundary_tagger(model_path: Path = None, device: str = "cuda"):
    ...
    if _semantic_tagger_instance is None:
        _semantic_tagger_instance = SemanticBoundaryTaggerLoader(
