...
class KoreanParticleMatcher:
    ...
    def __init__(self):
        ...
        self.particles = {
            ...
            "고어_관형": ["인", "하는", "할", "한", "하던", "하신", "함", "함이"],
            "고어_부사": ["이", "히", "게", "도록", "토록", "하여", "하되"],
            "고어_감탄": ["이여", "아", "어", "이로구나", "하는구나", "이로다"],
        }
            ...
            try:
                ...
                if tokens and len(tokens) > 0 and len(tokens[0]) > 0:
                    for token in tokens[0][0]:
                        ...
                        if (
                            pos.startswith("JK")
                            or pos.startswith("JX")
                            or pos.startswith("JC")
                        ):
    ...
    def calculate_particle_similarity(
        self,
        src_particles: List[Tuple[str, str, int]],
        tgt_particles: List[Tuple[str, str, int]],
    ) -> float:
        """
    ...
    def enhance_alignment_with_particles(self, alignments: List[Dict]) -> List[Dict]:
        ...
        for alignment in alignments:
            ...
            enhanced_similarity = (
                original_similarity
                + (particle_similarity * self._boost_pair)
                + archaic_bonus
            )
    ...
    def enhance_sa_single_units(self, src_unit: str, tgt_unit: str) -> Dict[str, Any]:
        ...
        try:
            ...
            final_similarity = embedding_similarity + (
                particle_similarity * self._boost_sa
            )
            ...
            return {
                ...
                "particle_match_quality": self._assess_match_quality(
                    particle_similarity
                ),
...
def enhance_sa_results_with_particles(
...
def find_sentence_boundaries_with_particles(text: str, target_count: int) -> List[str]:
    ...
    try:
        ...
        selected_positions = [
            sentence_end_positions[i * step] for i in range(target_count)
        ]
