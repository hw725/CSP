...
def evaluate_src_exact_subset(
    gold_df: pd.DataFrame, pred_df: pd.DataFrame, verbose: bool = False
) -> Dict[str, float]:
    """
    ...
    gold_df["_key"] = list(
        zip(
            gold_df["book_name"].fillna(""), gold_df["문장식별자"], gold_df["_src_norm"]
        )
    )
    pred_df["_key"] = list(
        zip(
            pred_df["book_name"].fillna(""), pred_df["문장식별자"], pred_df["_src_norm"]
        )
    )
    ...
    return {
        ...
        "target_sim_ge_90": (
            sum(1 for s in similarities if s >= 0.9) / len(similarities)
            if similarities
            else 0
        ),
        "target_sim_ge_80": (
            sum(1 for s in similarities if s >= 0.8) / len(similarities)
            if similarities
            else 0
        ),
    }
...
def print_results(results: Dict[str, float]):
    ...
    print("-" * 50)
    print(
        f"번역문 Precision: {results['target_precision']:.4f} ({results['target_precision']*100:.2f}%)"
    )
    print(
        f"번역문 Recall: {results['target_recall']:.4f} ({results['target_recall']*100:.2f}%)"
    )
    print(f"번역문 F1: {results['target_f1']:.4f} ({results['target_f1']*100:.2f}%)")
    print("-" * 50)
    print(
        f"번역문 평균 유사도: {results['target_avg_similarity']:.4f} ({results['target_avg_similarity']*100:.2f}%)"
    )
...
def main():
    ...
    parser.add_argument(
        ...
        "-o",
        help="결과 저장 경로",
        default="test_results/s2p_eval_result.csv",
    )
