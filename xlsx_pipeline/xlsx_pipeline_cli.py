...
class XLSXPipelineManager:
    ...
    def list_books(self):
        ...
        for i, book_info in enumerate(books, 1):
            ...
            status = []
            if exists.get("word"): status.append("구병렬")
            if exists.get("sentence"): status.append("문장병렬")
            if exists.get("paragraph"): status.append("문단병렬")
    ...
    def show_statistics(self, book_id: Optional[str] = None):
        ...
        if book_id:
            ...
            # 파일별 존재 여부 카운트
            word_count = sum(1 for b in all_stats["books"].values() if b.get("files_exist", {}).get("word", False))
            sent_count = sum(1 for b in all_stats["books"].values() if b.get("files_exist", {}).get("sentence", False))
            para_count = sum(1 for b in all_stats["books"].values() if b.get("files_exist", {}).get("paragraph", False))
            ...
            # 총 행 수
            total_words = sum(b.get("word_count", 0) for b in all_stats["books"].values())
            total_sents = sum(b.get("sentence_count", 0) for b in all_stats["books"].values())
            total_paras = sum(b.get("paragraph_count", 0) for b in all_stats["books"].values())
    ...
    def process_single_book(self, book_id: str, levels: List[str] = None, analysis: List[str] = None, project: str = None):
        """단일 책 처리"""
        print(f"\n🚀 책 처리 시작: {book_id}")
        print("=" * 80)
        ...
        try:
            ...
            # 결과 저장
            output_file = self.output_dir / f"{book_id}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    ...
    def batch_process(self, book_ids: List[str] = None, levels: List[str] = None, analysis: List[str] = None, project: str = None):
        """배치 처리"""
        if book_ids:
        ...
        try:
            ...
            success_count = sum(1 for r in results["results"].values() if "error" not in r)
            ...
            # 결과 저장
            output_file = self.output_dir / f"batch_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
            with open(output_file, 'w', encoding='utf-8') as f:
...
사용 예시:
  ...
  python xlsx_pipeline_cli.py batch --books 당송팔대가문초구양수1 당송팔대가문초구양수2
    ...
    parser.add_argument(
        "--xlsx-root",
        default="xlsx",
        help="Excel 파일 루트 디렉토리 (기본값: xlsx)"
    )
    ...
    parser.add_argument(
        "--book",
        help="처리할 책 ID (process 명령용)"
    )
    
    parser.add_argument(
        "--books",
        nargs="+",
        help="처리할 책 ID 리스트 (batch 명령용)"
    )
    ...
    valid_analysis = {"statistics", "similarity", "quality", "pa", "sa", "accuracy", "full"}
    normalized_analysis = []
    for item in args.analysis or []:
        for part in str(item).split(','):
            ...
            if p not in valid_analysis:
                parser.error(f"--analysis 잘못된 값: {p} (허용: {sorted(valid_analysis)})")
