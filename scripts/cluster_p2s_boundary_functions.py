...
def load_pa_sentence_pairs(path: Path) -> pd.DataFrame:
    ...
    for c in required:
        if c not in df.columns:
            raise SystemExit(
                f"입력 CSV에 {c} 컬럼이 없습니다. 사용 가능: {sorted(df.columns)}"
            )
    return df
...
def write_markdown_report(
    out_path: Path, df: pd.DataFrame, top_k: int, seed: int
) -> None:
    lines: list[str] = []
    ...
    for cid, gdf in df.groupby("cluster_id"):
        ...
        for _, r in gdf.head(int(top_k)).iterrows():
            lines.append(
                f"- book={r.get('book_name','')}, para={r.get('paragraph_id','')}"
            )
...
def main() -> None:
    ...
    p.add_argument(
        "--max-boundaries", type=int, default=20000, help="최대 경계 샘플 수"
    )
    ...
    p.add_argument(
        "--device-id", type=int, default=None, help="BGE 임베딩에 사용할 GPU id"
    )
    ...
    # 임베딩 캐시 옵션
    p.add_argument(
        "--load-embeddings", type=str, default=None, help="임베딩 캐시 로드 경로 (.npy)"
    )
    p.add_argument(
        "--save-embeddings", type=str, default=None, help="임베딩 캐시 저장 경로 (.npy)"
    )
    ...
    if args.load_embeddings and Path(args.load_embeddings).exists():
        ...
        X = compute_embeddings_batched(
            texts, batch_size=int(args.batch), device_id=args.device_id
        )
        X = _l2_normalize(X)
    ...
    km = MiniBatchKMeans(
        n_clusters=k, random_state=int(args.seed), batch_size=1024, n_init="auto"
    )
    ...
    def normalize_marker(m):
        if not m:
            return m
        if m in HYEONTO_REPLACE_MAP:
            return HYEONTO_REPLACE_MAP[m]
        if len(m) > 1 and (m.startswith("이") or m.startswith("으")):
            return m[1:]
        return m
    ...
    def extract_markers(text):
        if pd.isna(text):
            return ""
        markers = [
            normalize_marker(m.group("marker"))
    ...
    df_out["cluster_size"] = df_out["cluster_id"].map(
        lambda c: int(counts.get(int(c), 0))
    )
    df_out = df_out.sort_values(
        by=["cluster_size", "cluster_id"], ascending=[False, True], kind="mergesort"
    )
