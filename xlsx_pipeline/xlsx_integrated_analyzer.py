...
class XLSXIntegratedAnalyzer:
    ...
    def run_pa_analysis(
        ...
        book_id: str,
        use_global_progress: bool = False,
    ) -> Dict[str, Any]:
        """
        문단병렬 데이터에 PA 분석 실행
            ...
            if INTEGRITY_AVAILABLE and len(result_df) > 0:
                ...
                if not passed or analysis.get("total_losses", 0) > 0:
                    ...
                    if passed_after:
                        ...
                        logger.warning(
                            f"⚠️  부분 복원 완료 (여전히 {analysis_after.get('total_losses', 0)}건 손실)"
                        )

                # 무결성 리포트 저장
                if not losses_df.empty:
                    integrity_report_file = (
                        self.output_dir / f"{book_id}_p2s_integrity_report.csv"
                    )
                    losses_df.to_csv(
                        integrity_report_file, index=False, encoding="utf-8-sig"
                    )
                    logger.info(f"💾 무결성 리포트 저장: {integrity_report_file}")

            # 결과 저장
            output_file = self.output_dir / f"{book_id}_p2s_output.xlsx"
            result_df.to_excel(output_file, index=False)

            logger.info(f"✅ P2S 분석 완료: {output_file}")
            if not use_global_progress:
                finish_unified_progress()
    ...
    def run_sa_analysis(
        self, sentence_df: pd.DataFrame, book_id: str, use_global_progress: bool = False
    ) -> Dict[str, Any]:
        """
        문장병렬 데이터에 SA 분석 실행
        ...
        sa_embedder = os.getenv("CSP_S2P_EMBEDDER", "bge").lower()
        sa_use_semantic = os.getenv("CSP_S2P_USE_SEMANTIC", "1") != "0"
        sa_max_workers = int(os.getenv("CSP_S2P_WORKERS", "4"))
        sa_batch_size = int(os.getenv("CSP_S2P_BATCH", "100"))
        sa_device = os.getenv("CSP_DEVICE", "cuda")
        sa_device_id = 0 if sa_device.lower() == "cuda" else None
        logger.info(
            f"⚙️ SA 임베더: {sa_embedder}, device={sa_device}, semantic={sa_use_semantic}, workers={sa_max_workers}, batch={sa_batch_size}"
        )
        ...
        try:
            ...
            for idx, row in sentence_df.iterrows():
                ...
                for src_unit, tgt_unit, similarity in zip(
                    src_units, tgt_units, similarities
                ):
                    result_row = row.to_dict()
            ...
            # 활성화: CSP_S2P_RESTORE_INTEGRITY=1
            restore_sa_integrity = os.getenv("CSP_S2P_RESTORE_INTEGRITY", "0") == "1"
            ...
            if INTEGRITY_AVAILABLE and restore_sa_integrity and len(result_df) > 0:
                ...
                if not losses_df.empty:
                    integrity_report_file = (
                        self.output_dir / f"{book_id}_s2p_integrity_report.csv"
                    )
                    losses_df.to_csv(
                        integrity_report_file, index=False, encoding="utf-8-sig"
                    )
                ...
                logger.debug(f"⏭️  S2P 무결성 복원 스킵 (CSP_S2P_RESTORE_INTEGRITY=0)")

            # 결과 저장
            output_file = self.output_dir / f"{book_id}_s2p_output.xlsx"
            result_df.to_excel(output_file, index=False)

            logger.info(f"✅ S2P 분석 완료: {output_file}")
            if not use_global_progress:
                finish_unified_progress()
        ...
        if run_accuracy:
            ...
            if run_pa and results["results"]["pa"].get("success"):
                ...
                results["results"]["pa_accuracy"] = self.run_accuracy_evaluation(
                    str(word_file), pa_output, f"{book_id}_p2s", project
                )
            ...
            if run_sa and results["results"]["sa"].get("success"):
                ...
                results["results"]["sa_accuracy"] = self.run_accuracy_evaluation(
                    str(word_file), sa_output, f"{book_id}_s2p", project
                )
