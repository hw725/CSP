# Hyeonto 시각화 해석 가이드 (v6.9.4 Final)
...
- **왼쪽 노드 (Sentence)**: 문장의 종결 방식 및 문체적 특성 (예: 사서 논증, 문집 서사 등)
- **오른쪽 노드 (Phrase)**: 구 단위의 논리 관계 및 조사/접속 기능 (예: 조건 구문, 시간격 등)
...
- `reports/k4_embedding_overlay_2d.html` (2D)

### 해석 방법

- **점 색상**: K=4 클러스터 소속 (p0, p1, p2, p3)
...
| 영역 | 특징 | 해석 |
|:---|:---|:---|
| **중앙 밀집** | p1 클러스터 (사서 논증형) | 사서의 "문법적 북극점" |
| **상단 띠** | p0 클러스터 (역사서 서술) | 시간/편년체 구문 |
| **하단 분산** | p2, p3 클러스터 | 경전 정의 + 주석 설명 |
...
**핵심 발견**:
- 사서는 p1에서 Sentence/Phrase 모두 **13.4%+** 집중
- 역사서는 p0에 편중, 경전류는 p2/p3에 분산
...
- `reports/phrase_k4_normalized/phrase_cluster_profile.md`
...
**경로**: `reports/first_person_analysis/FIRST_PERSON_MARKER_ANALYSIS.md`
...
**경로**: `reports/genre_examples/GENRE_SPECIFIC_EXAMPLES.md`
