...
def setup_logging(verbose: bool = False):
    ...
    if not verbose:
        ...
        logging.getLogger("punctuation").setLevel(
            logging.ERROR
        )  # 🔧 무결성 경고 숨기기
        logging.getLogger("io_manager").setLevel(logging.ERROR)
        logging.getLogger("common.tokenizers").setLevel(
            logging.ERROR
        )  # 🔧 토크나이저 로깅 숨기기
...
def main():
    """메인 실행 함수"""
    parser = argparse.ArgumentParser(description="S2P: 한문-한국어 문장→구 분할 도구")
    ...
    parser.add_argument(
        ...
        nargs="?",
        default="input.xlsx",
        help="입력 엑셀 파일 경로 (기본: input.xlsx)",
    )
    parser.add_argument(
        "output_file",
        nargs="?",
        default="output.xlsx",
        help="출력 엑셀 파일 경로 (기본: output.xlsx)",
    )
    ...
    parser.add_argument(
        "--max-workers", type=int, default=4, help="최대 워커 수 (기본: 4)"
    )
    parser.add_argument(
        "--chunk-size",
        type=int,
    ...
    # 토크나이저 옵션
    parser.add_argument(
        "--min-src-tokens", type=int, default=1, help="원문 최소 토큰 수 (기본: 1)"
    )
    parser.add_argument(
        "--max-src-tokens", type=int, default=20, help="원문 최대 토큰 수 (기본: 20)"
    )
    parser.add_argument(
        "--min-tgt-tokens", type=int, default=1, help="번역문 최소 토큰 수 (기본: 1)"
    )
    parser.add_argument(
        "--max-tgt-tokens", type=int, default=40, help="번역문 최대 토큰 수 (기본: 40)"
    )
    # 가중치/스코어링 옵션 (실험적)
    parser.add_argument(
        "--dp-window",
    ...
    parser.add_argument(
        "--boundary-bonus", type=float, default=0.2, help="문장 경계 보너스 (기본: 0.2)"
    )
    parser.add_argument(
        "--particle-bonus",
        type=float,
    ...
    try:
        ...
        if not args.verbose:
            ...
            if success:
                ...
                # 🆕 자동 의미 기반 재정렬 후처리
                try:
                    from common.auto_semantic_reorderer import (
                        get_auto_semantic_reorderer,
                    )
                    ...
                    # 재정렬 결과를 포함한 파일 저장
                    output_with_reorder = args.output_file.replace(
                        ".xlsx", "_재정렬.xlsx"
                    )
